...
def _i_ex6_cell(ex, boat, margin=None, course=None):
...
def _ex6_cell(ex, hl=None, color=None):
    ...
    def _uv_candidates():
        ...
        def _exlane(jcd, rno, r=None, hl=None, color=None, closed=None):
            ...
            return ('<div style="text-align:left;white-space:nowrap"><div style="font-size:12px">%s</div>%s</div>'
                    % (_ex6_cell(ex, hl, color), _i_lane_html(ln, lr, settled=settled)))
        ...
        if ebuys:
            fh += ('<table style="margin-top:8px"><tr><th></th><th>締切</th><th>場R</th>'
                   '<th style="text-align:left">買い目（判定時オッズ）</th><th>展示タイム（1〜6号艇）・進入</th><th>8点平均EV</th><th>金額</th><th>結果</th></tr>')
        ...
        if fcands:
            fh += ('<div class="tsub" style="margin-top:8px;font-weight:700">未締切の候補（形の条件は満たした・締切5分前のオッズで買いか決まる）</div>'
                   ...
                   '<th>展示差</th><th>1号の総合力</th><th>展示タイム（1〜6号艇）・進入</th>'
        ...
        try:
            ...
            if fbuys:
                gh += ('<table style="margin-top:8px"><tr><th></th><th>締切</th><th>場R</th>'
                       '<th style="text-align:left">買い目（判定時オッズ・合成EV）</th><th>展示タイム（1〜6号艇）・進入</th><th>金額</th><th>結果</th></tr>')
        ...
        try:
            ...
            if _gb:
                g8 += ('<table style="margin-top:8px"><tr><th></th><th>締切</th><th>場R</th><th>買い目</th>'
                       '<th>判定時→決定時</th><th>③集中・②頭<br><span style="font-weight:400">(v121 記録のみ)</span></th><th>展示タイム（1〜6号艇）・進入</th><th>金額</th><th>結果</th></tr>')
                for ca, r, gv in _gb:
                    pk = (gv.get("picks") or [{}])[0]
                    if r.get("status") == "settled" and r.get("win_combo"):
            ...
            if _gc:
                g8 += ('<div class="tsub" style="margin-top:8px;font-weight:700">候補（判定済み・決定待ち）</div>'
                       '<table style="margin-top:4px"><tr><th>締切</th><th>場R</th><th>監視中の組</th><th>展示タイム（1〜6号艇）・進入</th><th>決定まで</th></tr>')
                for ca, r, gv in _gc:
                    try:
                        _hh, _mm = [int(x) for x in ca.split(":")[:2]]
        ...
        try:
            ...
            if _ib:
                i8 += ('<table style="margin-top:8px"><tr><th></th><th>締切</th><th>場R</th><th>買い目</th><th>展示タイム（1〜6号艇）</th><th>金額</th><th>結果</th></tr>')
                for ca, r, iv in _ib:
            ...
            if icands:
                i8 += ('<div class="tsub" style="margin-top:8px;font-weight:700">買い予定（展示で0.1秒以上速い・判定時に確定）</div>'
                       '<table style="margin-top:4px"><tr><th>締切</th><th>場R</th><th>買い目</th><th>展示タイム（1〜6号艇）</th><th>確定まで</th></tr>')
        ...
        if n == 0:
            ...
            for c in cands:
                ...
                ex6_s = ('<div style="text-align:left;white-space:nowrap"><div style="font-size:12px">%s</div>%s</div>'
                         % (_ex6_cell(_ex6, 1, color), _i_lane_html(_ln6, _lr6, settled=c["done"])))
                if ld is None:
                    lane_s = ''
                elif ld["wakunari"]:
                ...
                if _RARE_BOX_LIVE and field == "rare20" and c["done"]:   # v126b: BOXの当否
                    ...
                    for _bk, _nm in (("box2", "2連単BOX"), ("box4", "3連単BOX")):
                        ...
                        if any(p.get("combo") == _src[0] for p in _bx["picks"]):
                            res += '<br><span class="res-hit">%s 的中 %s（%s円/100円）</span>' % (_nm, _src[0], f"{int(_src[1]):,}")
                        else:
