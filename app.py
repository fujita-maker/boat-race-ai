...
def final_page():
    ...
    uv_shadow = ('<div class="fcard" style="border-color:#dbe2ea">'
                 ...
                 '&#128203; 記録のみのバケット（買わない・検証用）　%d件</div>' % ((35 if _MIDEV_DISPLAY else 31 + len(_MIDEV_SHADOWS)) + (0 if _F_LIVE else 1) + (0 if _B_LIVE else 1) + 1 + (0 if _C_LIVE else 1))
    ...
    def card(r):
        ...
        if isbuy:
            ...
            h += '<div class="tsub" style="margin:4px 0">実弾の枠（A案・D案・E案・F案・波乱）はどれも条件に当てはまらないため買いません。%s</div>' % (
