...
st.set_page_config(page_title="WDJ Boat Race AI v9", layout="wide")
...
# ---------- UI ----------
st.title("🚤 WDJ ボートレースAI Web版 v9")
st.caption(
    "公式・梅吉AI・ポセイドンを統合。結果を保持し、見送りでも参考買い目を必ず表示し、"
    "WDJ115-v2条件に適合したレースだけ5,000円で仮想購入します。"
)
st.info("V9完成版：結果を画面に保持し、見送りでも参考買い目6〜8点を最上部に必ず表示します。")
...
with tabs[0]:
    ...
    if run_prediction:
        ...
        with st.spinner("公式・梅吉AI・ポセイドンを取得し、予想中…"):
            ...
            try:
                ...
                safe_official = {
                    "errors": official.get("errors", []) + [f"予想計算: {exc}"],
                    "list": {"text": "", "tables": [], "html": ""},
                    "before": {"text": "", "tables": [], "html": ""},
                    "odds": {"text": "", "tables": [], "html": ""},
                    "urls": official.get("urls", {}),
                }
                safe_poseidon = {
                    "text": "", "error": poseidon.get("error"),
                    "url": poseidon.get("url", ""), "html": "", "tables": [],
                }
                safe_umepyon = {
                    "text": "", "error": umepyon.get("error"),
                    "url": umepyon.get("url", ""), "html": "", "tables": [],
                }
                calc = build_prediction(safe_official, safe_poseidon, safe_umepyon)
        ...
        # ボタン後の再描画でも結果が消えないよう保持
        st.session_state["v9_result"] = {
            "calc": calc,
            "official": official,
            "poseidon": poseidon,
            "umepyon": umepyon,
    ...
    result = st.session_state.get("v9_result")
    ...
    if result:
        ...
        result_race_no = result["race_no"]

        star_text = "★" * calc["stars"] + "☆" * (5 - calc["stars"])
        st.success(f"{result_venue}{result_race_no}R｜締切前固定｜{calc['action']}")
        ...
        # V9の最重要表示：詳細より先に必ず買い目を出す
        st.subheader("🎯 参考買い目（見送りでも必ず表示）")
        compact_rows = []
        for row in calc["predictions"][:8]:
            odds_text = f"{row.get('odds', 0):.1f}倍" if row.get("odds", 0) else "未取得"
        ...
        if calc["action"] == "買い":
            st.success(f"購入条件適合：合計{calc['budget']:,}円で仮想購入します。")
        else:
            st.warning("購入は0円で見送りますが、上の買い目はAIの固定予想です。")

        if result.get("save_error"):
            st.warning(f"履歴保存だけ失敗しましたが、予想表示には影響ありません：{result['save_error']}")
        ...
        exhibition_rows = meta.get("exhibition_rows", [])
        if exhibition_rows:
            st.dataframe(
                pd.DataFrame(exhibition_rows),
                hide_index=True,
                use_container_width=True,
            )
        ...
        with st.expander("詳細データ（期待値・市場確率など）"):
            ...
            available_columns = [c for c in columns if c in prediction_df.columns]
        ...
        st.subheader("総合判断理由")
        st.write(calc.get("reason", "参考予想として固定しました。"))
