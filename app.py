...
_IX_LIVE = True
_IX_UNIT = 1000
_IX_PLANS = (("I2", "I2案", "2連単 H→展示2位"), ("I3", "I3案", "2連単 H→勝率上位2"), ("I4", "I4案", "3連単 H-勝率上位2-上位3"))
...
def ix_pick(boats, iv):
    ...
    try:
        ...
        out = {"boat": H, "ex2": ex2[:1], "wr3": wr[:3]}
        if ex2:
            out["I2"] = {"picks": [{"combo": "%d-%d" % (H, ex2[0]), "type": "2連単", "stake": _IX_UNIT}]}
        out["I3"] = {"picks": [{"combo": "%d-%d" % (H, b), "type": "2連単", "stake": _IX_UNIT} for b in wr[:2]]}
        out["I4"] = {"picks": [{"combo": "%d-%d-%d" % (H, a, c), "type": "3連単", "stake": _IX_UNIT}
                               for a in wr[:2] for c in wr[:3] if c != a]}
        return out
    except Exception as _e:
    ...
    for k, lab, desc in _IX_PLANS:
        ...
        h += ('<div class="tsub" style="margin-top:2px"><b style="color:%s">%s %s</b>　%s：<span class="combo">%s</span>（%d点×%s円）%s</div>'
              % (_I_COLOR, lab, "買い" if _IX_LIVE else "（記録のみ）", desc,
                 "・".join(p["combo"] for p in v["picks"]), len(v["picks"]), f"{_IX_UNIT:,}", tail))
    return h
...
def final_page():
    ...
    def _live_pending(plan):
        ...
        for r, u in uv_today:
            ...
            try:
                ...
                elif plan in ("I2", "I3", "I4"):   # v123
                    v = (u.get("ixplan") or {}).get(plan) or {}
                    items = [(p.get("combo"), None, fnum(p.get("stake"))) for p in (v.get("picks") or [])]
    ...
    uv_live = ('<div class="fcard" style="border-color:#1c7a38;background:#f4fbf6">'
               ...
               + "".join(_uv_row("%s：I案の艇（H）を1着にした %s（%d点×%d円）" % (lab, desc, n_, _IX_UNIT),
                                 _ixplan_stats(k), live=True, color=_I_COLOR, plan=k)
                         for (k, lab, desc), n_ in zip(_IX_PLANS, (1, 2, 4)) if _IX_LIVE)
    ...
    def card(r):
        ...
        _ixpk = [(lab, p) for k, lab, _ in _IX_PLANS for p in ((_ix_c.get(k) or {}).get("picks") or [])]
        ...
        if isbuy:
            ...
            for _xl, p in _ixpk:
                stk = fnum(p.get("stake")); _tot += stk
                h += '<tr><td style="color:%s;font-weight:700">%s</td><td class="combo">%s %s</td><td>—</td><td>%s円</td><td>%s</td></tr>' % (
                    _I_COLOR, _xl, p.get("type"), p.get("combo"), f"{int(stk):,}", "オッズ次第")
            for p in _gpk:
                o = p.get("odds"); stk = fnum(p.get("stake")); _tot += stk
                pay = (fnum(o) * stk) if (o is not None and stk) else None
            ...
            if _ib:
                ...
                for ca, r, iv in _ib:
                    ...
                    for _k, _xl, _ in _IX_PLANS:
                        ...
                        _xc += '<div style="font-size:11px"><b>%s</b> %s</div>' % (_xl, "・".join(p["combo"] for p in _v["picks"]))
                        if str(_v.get("hit", "")) != "":
