...
def final_page():
    ...
    uv_kpi = (_uv_row("≥40（前モデル・過去BT71%）", uv40)
              + _uv_row("≥55（厳選・過去BT90%）", uv55))
    ...
    def agg_uv(rs, field):
        ...
        for r in rs:
            try:
                ...
                u = {}
            dec = u.get(field, u.get("decision"))
...
def undervalue_pick(race, odds_map):
    ...
    odds_ok = (max_odds >= 15)
    # 2つの閾値を並走: ≥40(前モデル・回収率71%) と ≥55(厳選・回収率90%)。両方の買い/見送りを返す。
    dec40 = "買い" if (odds_map and gap[X] >= 3 and mot[X] >= 40 and odds_ok) else "見送り"
    dec55 = "買い" if (odds_map and gap[X] >= 3 and mot[X] >= 55 and odds_ok) else "見送り"
    ...
    return {"decision": dec55, "dec40": dec40, "dec55": dec55,
