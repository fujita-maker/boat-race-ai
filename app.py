...
def _form_watch_note(gv):
    """v120: E案が買いでも、記録のみの見送り候補に当たっていれば注意書きを付ける（判定は変えない）。"""
    ws = []
    if gv.get("watch_a"):
        ws.append("見送り候補A（1号艇%s・2〜6号艇のA級%s人）" % (gv.get("cls1") or "-", gv.get("nA")))
    if gv.get("watch_loc"):
        ws.append("見送り候補B（当地勝率1位の%s号艇が2着候補の外）" % gv.get("watch_loc"))
    if not ws:
        return ""
    return ('<div style="margin-top:4px;font-size:12px;color:#b8860b">&#9888;&#65039; %s'
            '<span style="color:#8a97a5">　※記録のみ。過去の成績が悪い形（1号艇A級 25.2%%・A級4人以上 的中0）。'
            '買うかは判断してください</span></div>') % "／".join(ws)
...
def _form_line(uv):
    ...
    if gv.get("flag"):
        return ('<div class="tsub" style="margin-top:4px;background:%s;border:1px solid %s;'
                ...
                '　%s%s%s</div>') % (_FORM_BG, _FORM_COLOR, _FORM_COLOR, _FORM_COLOR_D,
                                  _fnum(gv.get("ev1")), _fnum(gv.get("board_min")),
                                  _fnum(gv.get("min_odds")),
                                  "・".join(str(x) for x in gv["second"]),
                                  f"{_FORM_UNIT:,}", body, tail, _form_watch_note(gv))
...
_B_LIVE = False
# v124(2026-09-23 藤田指示「そうして」＝F案を記録のみ・B案を実弾に戻す): B案の買い目は A案と共通で、
#   v85 の「オッズ150倍以下 かつ 1点EV≥1.5」の点だけ（_UV_LEG_ODDS_CAP/_UV_LEG_EV_FLOOR）。
#   根拠: uvp_both 8/28〜9/22 266R・的中16・実払戻で253.2%（大穴3本抜き132.3・上位3日除外109.9・日95%下限92.3）。
#   ただし閾値1.5は同じ期間で選んだ。条件を決めた後(9/19〜)は26R・的中1・99.4%。[[boat-race-uvp-both-vs-f-0923]]
_B_LIVE = True
...
# v124(2026-09-23): F案を記録のみに戻す。実弾34R・54.2%、買い目の99%が「モデル確率÷市場確率≥2」＝モデルの過大評価に乗っている。
#   [[boat-race-fplan-filters-0923]]。判定・記録・精算は継続。
_F_LIVE = False
...
_CD_SCRIPT = (
    ...
    "if(d<=0){e.textContent=e.getAttribute('data-done')||'判定中…';e.style.color=e.getAttribute('data-done')?'#b8860b':'#1c7a38';return;}"
...
def _g_decide(gv, odds_now, now_s):
    ...
    gv.update(_g_filters(best, _chs, _t1))
    if best[1] <= -_G_DROP:
        _pk = [{"combo": best[0], "odds0": best[2], "odds": best[3], "drop": round(best[1], 3), "stake": _G_UNIT}]
        _why = "%s が %.1f倍→%.1f倍（%d%%下落）" % (best[0], best[2], best[3], round(-100 * best[1]))
        # v122: 元のG案の買いは g0_picks に必ず残す（記録のみ・比較用）
        gv["g0_picks"] = _pk
        _ng = []
        if _G_REQ_CONC and not gv.get("g_conc"):
            _ng.append("③お金が1点に集中していない（2番目との差%.1fpt＜%dpt）"
                       % (100 * (gv.get("gap") or 0), round(100 * _G_CONC_GAP)))
        if _G_REQ_NON1 and not gv.get("g_non1"):
            _ng.append("②1号艇が頭")
        if _ng:
            gv.update(status="見送り", reason=_why + " だが " + "・".join(_ng) + "（元のG案なら買い）")
        else:
            gv.update(status="買い", reason=_why, picks=_pk)
    else:
        gv.update(status="見送り", reason="一番下がった %s でも %d%%（3割に届かず）" % (best[0], round(-100 * best[1])))
    return gv


# ============ v121(2026-09-23 藤田指示「③と②両方で進めて」): G案の絞り込み2つを記録のみで並走 ============
#   事前登録: 仮説台帳 H7/H8（[[boat-race-gplan-filters-0923]]）。後知恵版G 1,898R（判定時→最終）で
#     ③お金の集中（1番下がった組と2番目の差≥10pt）: 1,024R・的中84・117.1%（G全体111.8%）・大穴3本抜き107.9・上位3日除外101.6・95%下限96.1
#     ②1号艇以外が頭: 224R・的中20・160.8%・95%下限87.5（的中本数不足）
#   本番は「判定時→締切約5分前」で測るので、ここで同じ測り方で前向きに確かめる。G案の買い判定は変えない。
_G_CONC_GAP = 0.10
# v122(2026-09-23 藤田指示「今のG案を②③のG案にしたい」): 実際の買いを ③かつ② を満たすものだけにする。
#   元のG案（3割以上下落の最大1点）は gplan.g0_picks / g0_hit / g0_ret に記録のみで残す。
#   後知恵版1,898Rでの③かつ②: 142R・的中14・183.0%・大穴3本抜き126.0・上位3日除外92.3・95%下限76.2（採用基準は未達・藤田判断で実弾化）。
_G_REQ_CONC = True
_G_REQ_NON1 = True


def _g_filters(best, chs, t1):
    """v121: G案の決定時に③（お金の集中）・②（頭の艇番）とH6r用の数字を付ける。記録のみ。"""
    try:
        rest = sorted(chs)[1:] if len(chs) > 1 else []
        drop2 = rest[0] if rest else None
        gap = (drop2 - best[1]) if drop2 is not None else None
        _sc = sorted(chs); _n = len(_sc)
        med = ((_sc[_n // 2] if _n % 2 else (_sc[_n // 2 - 1] + _sc[_n // 2]) / 2.0) if _n else None)
        head = int(str(best[0]).split("-")[0])
        return {"t1odds": t1, "drop2": (round(drop2, 3) if drop2 is not None else None),
                "gap": (round(gap, 3) if gap is not None else None),
                "med": (round(med, 3) if med is not None else None), "head": head,
                "g_conc": bool(gap is None or gap >= _G_CONC_GAP), "g_non1": head != 1,
                "rel": (round(best[1] - med, 3) if med is not None else None)}
    except Exception as _e:
        print("[v121 g_filters]", _e, flush=True)
        return {}


def _g_filter_tag(gv):
    """v121: カード用の小さな表示（③・②）。"""
    if "g_conc" not in gv:
        return ""
    c = ('<span style="color:#1c7a38">③集中○</span>' if gv.get("g_conc") else '<span style="color:#8a97a5">③集中×</span>')
    g2 = ("（2番目 %d%%下落・差%dpt）" % (round(-100 * gv["drop2"]), round(100 * gv["gap"]))
          if gv.get("drop2") is not None else "（2番目なし）")
    n = ('<span style="color:#1c7a38">②%s号艇頭○</span>' % gv.get("head") if gv.get("g_non1")
         else '<span style="color:#8a97a5">②1号艇頭×</span>')
    return '　<span style="font-size:11px">[記録のみ] %s%s・%s</span>' % (c, g2, n)
...
def _g_line(uv, closed_hm=None):
    ...
    if stt == "買い":
        ...
        return ('<div class="tsub" style="margin-top:2px">%s <b style="color:#1c7a38">決定：買い</b>　'
                '<span class="combo">%s</span>（%s倍→%s倍・%d%%下落）1点×%s円%s%s</div>'
                % (lab, pk.get("combo"), pk.get("odds0"), pk.get("odds"), round(-100 * _fnum(pk.get("drop"))),
                   f"{_G_UNIT:,}", tail, _g_filter_tag(gv)))
    return '<div class="tsub" style="margin-top:2px">%s 見送り<span style="color:#8a97a5">（%s）</span>%s</div>' % (
        lab, gv.get("reason", ""), _g_filter_tag(gv) if gv.get("g0_picks") else "")
...
# ============ v123(2026-09-23 藤田指示「実際に買う枠にしたい」): I案の艇を1着にした2連単・3連単 3案 ============
#   I2案: 2連単 H→展示2位（1点）／I3案: 2連単 H→全国勝率の上位2艇（2点）／I4案: 3連単 H-勝率上位2-上位3（4点）。1点1,000円。
#   H＝I案の艇（展示で単独最速かつ2番手より0.10秒以上速い）。I案が立ったレースだけ。
#   実弾8R(9/21〜22): I2 261.2%・I3 146.9%・I4 125.6%（いずれも的中1〜2本）。
#   ⚠ 2026-01-01〜09-22の1,780Rでは I2 67.8%・I3 77.1%・I4 66.0%（単勝78.2%より下）。藤田判断で実弾。
#   [[boat-race-iplan-formation-0923]]
_IX_LIVE = True
_IX_UNIT = 1000
# v131(2026-09-23 藤田指示): 旧I2(H→展示2位)と旧I3(H→勝率上位2)を1つにまとめて「I2案」(2連単 H-展示2位・勝率上位2、重複は1点)、
#   I4案は「3連単 H-勝率上位2-全」(8点)に変更。表示は 1-2,5,6 ／ 1-2,6-全 の形。
_IX_PLANS = (("I2", "I2案", "2連単 H→展示2位・勝率上位2"), ("I4", "I4案", "3連単 H→勝率上位2→全"))


def _ix_from(H, ex2, wr3):
    """v131: I2案(2連単 H-[展示2位＋勝率上位2]・重複は1点) と I4案(3連単 H-[勝率上位2]-全)。"""
    H = int(H)
    ex2 = [int(x) for x in (ex2 or [])][:1]
    wr3 = [int(x) for x in (wr3 or [])][:3]
    sec = []
    for b in ex2 + wr3[:2]:
        if b != H and b not in sec:
            sec.append(b)
    sec.sort()
    top2 = sorted(wr3[:2])
    out = {"v": 2, "boat": H, "ex2": ex2, "wr3": wr3}
    if sec:
        out["I2"] = {"picks": [{"combo": "%d-%d" % (H, b), "type": "2連単", "stake": _IX_UNIT} for b in sec]}
    if top2:
        out["I4"] = {"picks": [{"combo": "%d-%d-%d" % (H, a, c), "type": "3連単", "stake": _IX_UNIT}
                               for a in top2 for c in range(1, 7) if c not in (H, a)]}
    return out


def _ix_upgrade(xv):
    """v131: 旧形式(I2/I3 別々)の記録を「まとめて表示」するだけ。買い目・当否は変えない。
    I2＝旧I2＋旧I3 の2連単(同じ組は1点)。当否は旧の結果をそのまま引き継ぐ。
    I4＝すでに判定済みのレースは旧4点のまま(実際に買った買い目を書き換えない)。"""
    if not xv or xv.get("v") == 2 or not xv.get("boat"):
        return xv
    try:
        out = dict(xv); out["v"] = 2
        o2 = xv.get("I2") or {}; o3 = xv.get("I3") or {}
        picks = []; seen = set()
        for p in (o2.get("picks") or []) + (o3.get("picks") or []):
            if p.get("combo") not in seen:
                seen.add(p.get("combo")); picks.append(p)
        picks.sort(key=lambda p: p.get("combo"))
        if picks:
            n2 = {"picks": picks}
            h2, h3 = str(o2.get("hit", "")), str(o3.get("hit", ""))
            if h2 != "" or h3 != "":
                n2["hit"] = 1 if "1" in (h2, h3) else 0
                n2["ret"] = int(max(_fnum(o2.get("ret")), _fnum(o3.get("ret"))))
            out["I2"] = n2
        out.pop("I3", None)
        return out
    except Exception:
...
def _ix_fmt(k, v):
    """v131: 表示用。I2 → 1-2,5,6 ／ I4 → 1-2,6-全（旧4点の記録は 1-2,6-2,4,6 の形）。"""
    ps = [p.get("combo", "") for p in (v or {}).get("picks") or []]
    if not ps:
        return ""
    H = ps[0].split("-")[0]
    sec = []; thr = []
    for c in ps:
        x = c.split("-")
        if len(x) >= 2 and x[1] not in sec:
            sec.append(x[1])
        if len(x) >= 3 and x[2] not in thr:
            thr.append(x[2])
    sec.sort(); thr.sort()
    s = "%s-%s" % (H, ",".join(sec))
    if thr:
        full = all(len([t for t in range(1, 7) if str(t) not in (H, b)]) ==
                   len([c for c in ps if c.split("-")[1] == b]) for b in sec)
        if not full:
            return "・".join(ps)   # 旧4点など流しでない記録は買い目をそのまま並べる
        s += "-全"
    return s


def ix_pick(boats, iv):
    """v123: I案が立ったとき、その艇を1着にした I2/I3/I4 の買い目を作る。"""
    try:
        if not (iv or {}).get("flag"):
            return {}
        H = int(iv.get("boat"))
        ex, nat = {}, {}
        for b in boats or []:
            ...
            try:
                ...
                pass
            try:
                nat[f] = float(b.get("nat") or 0)
            except (TypeError, ValueError):
                nat[f] = 0.0
        oth = [f for f in range(1, 7) if f != H]
        ex2 = sorted([f for f in oth if f in ex], key=lambda f: ex[f])
        wr = sorted(oth, key=lambda f: (-nat.get(f, 0), f))
        return _ix_from(H, ex2[:1], wr[:3])
    except Exception as _e:
        print("[v123 ix_pick]", _e, flush=True)
        return {}


def _ix_settle(xv, ex_pay, tri_pay):
    """v123: I2/I3/I4 の精算。ex_pay/tri_pay=(組, 100円あたり払戻) または None。"""
    for k, _, _ in _IX_PLANS:
        v = xv.get(k)
        if not v or not v.get("picks"):
            continue
        h = 0; rt = 0.0; ok = True
        for p in v["picks"]:
            src = tri_pay if p.get("type") == "3連単" else ex_pay
            if src is None:
                ok = False; break
            if p.get("combo") == src[0]:
                h = 1; rt += _fnum(p.get("stake")) * (float(src[1]) / 100.0)
        if ok:
            v["hit"] = h; v["ret"] = int(rt)
    return xv


def _ix_line(uv):
    """v123: カード内の I2/I3/I4 の行。"""
    xv = (uv or {}).get("ixplan") or {}
    if not xv:
        return ""
    h = ""
    for k, lab, desc in _IX_PLANS:
        v = xv.get(k) or {}
        if not v.get("picks"):
            continue
        tail = ""
        if str(v.get("hit", "")) != "":
            tail = ('　<span class="res-hit">的中（払戻%s円）</span>' % f"{int(_fnum(v.get('ret'))):,}"
                    if str(v.get("hit")) == "1" else '　<span class="res-miss">不的中</span>')
        h += ('<div class="tsub" style="margin-top:2px"><b style="color:%s">%s %s</b>　%s：<span class="combo">%s</span>（%d点×%s円）%s</div>'
              % (_I_COLOR, lab, "買い" if _IX_LIVE else "（記録のみ）", desc,
                 _ix_fmt(k, v), len(v["picks"]), f"{_IX_UNIT:,}", tail))
    return h


# ==== v129(2026-09-23 藤田指摘「結果がもう出てるのに反映されるのが遅い」) ====
#   精算は boatraceopenapi(GitHub上のJSON) の結果を待っていたが、あちらは公式より10〜30分遅れる。
#   締切から1分過ぎても結果が無いレースは、公式の結果ページ(raceresult)を直接読んで先に精算する。
#   後で openapi に結果が載ったら突き合わせ、食い違っていれば精算をやり直す(_settle_src で管理)。
_RES_CACHE = {}
_RES_NAMES = {"3連単": "trifecta", "3連複": "trio", "2連単": "exacta", "2連複": "quinella",
              "拡連複": "quinella_place", "単勝": "win", "複勝": "place"}


def fetch_result_official(hd, jcd, rno):
    """公式 raceresult ページの払戻 → openapi と同じ形の payouts dict。3連単が取れなければ None。"""
    key = (str(hd), int(jcd), int(rno))
    ent = _RES_CACHE.get(key)
    if ent and (ent[1] is not None or time.time() - ent[0] < 50):
        return ent[1]
    pay = None
    try:
        url = "https://boatrace.jp/owpc/pc/race/raceresult?rno=%d&jcd=%02d&hd=%s" % (int(rno), int(jcd), hd)
        rq = requests.get(url, headers=UA, timeout=8)
        if rq.status_code == 200:
            pay = _parse_result_payouts(rq.text)
    except Exception:
        pay = None
    _RES_CACHE[key] = (time.time(), pay)
    return pay


def _parse_result_payouts(html):
    ...
    for tr in soup.find_all("tr"):
        ...
        if not cells:
            continue
        if cells[0] in _RES_NAMES:
            cur = _RES_NAMES[cells[0]]
            cells = cells[1:]
        elif any(c in _RES_NAMES for c in cells):
            cur = None
            continue
        if cur is None:
            continue
        ...
        for c in cells:
            c2 = c.replace(" ", "")
            if combo is None and re.fullmatch(r"[1-6](?:[-=][1-6]){0,2}", c2):
                combo = c2
            m = re.fullmatch(r"[¥￥]([\d,]+)", c2)
            if amt is None and m:
                amt = int(m.group(1).replace(",", ""))
        if combo and amt:
            out.setdefault(cur, []).append({"combination": combo, "amount": amt, "label": None})
    tri = out.get("trifecta") or []
    if not tri or not re.fullmatch(r"[1-6]-[1-6]-[1-6]", tri[0]["combination"]) or len(set(tri[0]["combination"].split("-"))) != 3:
        return None
    ordered = {}
    for k in ("trifecta", "trio", "exacta", "quinella", "quinella_place", "win", "place"):
        if out.get(k):
            ordered[k] = out[k]
    return ordered


def _race_with_result(rc, hd, jcd, rno):
    """openapi に結果が無ければ公式から払戻を補った rc を返す(無理なら元の rc)。"""
    try:
        if rc is not None and _payouts(rc)[1] is not None:
            return rc
    except Exception:
        pass
    pay = fetch_result_official(hd, jcd, rno)
    if not pay:
        return rc
    rc2 = dict(rc or {})
    rc2["result"] = {"payouts": pay, "_src": "official"}
    return rc2
...
# ==== v128(2026-09-23 藤田指示「各号艇に級・全国勝率・全国2連率を全部の案にのせて・見やすく」) ====
#   E/F/G/I案・波乱の目の「展示タイム（1〜6号艇）」セルを、1艇1行の小さな表に作り直す。
#   列: 艇(公式の艇色) / 級 / 全国勝率 / 全国2連率 / 展示タイム(順位)。最速=緑太字・最遅=赤。
#   強調する艇(E案の展示最速・I案の買う艇)は行ごと色付き。展示前でも級・勝率・2連率は出す。
_BOAT_BG = {1: ("#ffffff", "#222", "#b7bec6"), 2: ("#222222", "#fff", "#222"), 3: ("#e23b3b", "#fff", "#e23b3b"),
            4: ("#2f6fd6", "#fff", "#2f6fd6"), 5: ("#f2d21b", "#222", "#d9bb12"), 6: ("#2e9e4f", "#fff", "#2e9e4f")}
_CLS_STYLE = {"A1": "background:#b23b3b;color:#fff", "A2": "background:#e8a33d;color:#fff",
              "B1": "background:#e8ecf1;color:#2c3742", "B2": "background:#f4f6f8;color:#8a97a5"}


def _boat_info(rc):
    """出走表から {枠: {"cls","nat","n2"}}。取れなければ {}。"""
    out = {}
    if rc is None:
        return out
    try:
        ents = []
        _collect_with_key(rc, "national_win_rate", ents)
        for e in ents:
            n = int(e.get("entry_number") or 0)
            if 1 <= n <= 6:
                out[n] = {"cls": e.get("rank_number_source") or "", "nat": e.get("national_win_rate"),
                          "n2": e.get("national_top_2_percent")}
    except Exception:
        return {}
    return out


def _boat6_table(ex, info=None, hl=None, color=None):
    """6艇の小さな表。ex={枠:展示タイム}、info=_boat_info(rc)。"""
    info = info or {}
    try:
        hl = int(hl) if hl not in (None, "") else None
    except (TypeError, ValueError):
        hl = None
    ex = {int(k): float(v) for k, v in (ex or {}).items() if v not in (None, "") and _fnum(v) > 0}
    have_ex = len(ex) >= 4
    best = min(ex.values()) if have_ex else None
    worst = max(ex.values()) if have_ex else None
    nat_best = max((_fnum(i.get("nat")) for i in info.values()), default=0)
    hc = color or "#1c7a38"
    th = 'style="padding:1px 5px;font-size:10px;color:#8a97a5;font-weight:600;text-align:%s"'
    h = ('<table style="border-collapse:collapse;margin:0;width:auto;font-size:12px;line-height:1.35">'
         '<tr><th %s>艇</th><th %s>級</th><th %s>勝率</th><th %s>2連率</th><th %s>展示</th></tr>'
         % (th % "center", th % "center", th % "right", th % "right", th % "right"))
    for f in range(1, 7):
        bg, fg, bd = _BOAT_BG[f]
        it = info.get(f) or {}
        cls = str(it.get("cls") or "")
        nat = _fnum(it.get("nat")); n2 = _fnum(it.get("n2"))
        row_bg = "background:%s18;box-shadow:inset 3px 0 0 %s;" % (hc, hc) if f == hl else ""
        chip = ('<span style="display:inline-block;width:18px;height:18px;line-height:18px;border-radius:4px;'
                'text-align:center;font-weight:800;font-size:11px;background:%s;color:%s;border:1px solid %s">%d</span>'
                % (bg, fg, bd, f))
        cls_s = ('<span style="display:inline-block;min-width:22px;border-radius:4px;padding:0 3px;font-size:10px;'
                 'font-weight:700;text-align:center;%s">%s</span>' % (_CLS_STYLE.get(cls, "color:#8a97a5"), cls or "—"))
        nat_s = ("<b>%.2f</b>" % nat if nat and nat >= nat_best - 1e-9 else ("%.2f" % nat if nat else "—"))
        n2_s = ("%.1f%%" % n2) if n2 else "—"
        v = ex.get(f)
        if v is None:
            ex_s = '<span style="color:#8a97a5">%s</span>' % ("—" if have_ex else "待ち")
        else:
            rk = 1 + sum(1 for g, w in ex.items() if g != f and w < v)
            if have_ex and v <= best + 1e-9:
                st = "color:#1c7a38;font-weight:800"
            elif have_ex and v >= worst - 1e-9:
                st = "color:#b23b3b;font-weight:700"
            else:
                st = "color:#2c3742"
            ex_s = '<span style="%s">%.2f</span><span style="font-size:10px;color:#8a97a5"> %d位</span>' % (st, v, rk)
        td = 'style="padding:1px 5px;text-align:%s;white-space:nowrap;border-top:1px solid #eef1f4"'
        h += ('<tr style="%s"><td %s>%s</td><td %s>%s</td><td %s>%s</td><td %s>%s</td><td %s>%s</td></tr>'
              % (row_bg, td % "center", chip, td % "center", cls_s, td % "right", nat_s,
                 td % "right", n2_s, td % "right", ex_s))
    return h + "</table>"

def _i_ex6_cell(ex, boat, margin=None, course=None, rc=None):
    """v112(2026-09-21 藤田指示「I案は展示タイムが主なので1〜6号艇の展示タイムもここに出して」):
    I案ブロックの「展示」セル。1〜6号艇の展示タイムを並べ、買う艇を I案の色で太字＋枠、最遅を赤にする。
    ex = {枠番(int): 展示タイム(float)}。4艇未満しか無ければ従来の1艇表示にフォールバック。"""
    try:
        boat = int(boat)
    except (TypeError, ValueError):
        boat = None
    ...
    if margin is not None:
        tail = '<div style="font-size:11px;color:#5c6b7a;margin-top:2px">2番手と<b>%.2f秒差</b>%s</div>' % (
            _fnum(margin), ("・%sコース" % course) if course and str(course) != str(boat) else "")
    if rc is not None:   # v128: 級・勝率・2連率つきの表
        return _boat6_table(ex, _boat_info(rc), hl=boat, color=_I_COLOR) + tail
    if len(ex) < 4:
        v = ex.get(boat) if boat else None
        return ("%.2f" % v if v else "—") + tail
    worst = max(ex.values())
    cells = []
    for f in range(1, 7):
        v = ex.get(f)
        if v is None:
            cells.append('<span style="color:#8a97a5">%d号 —</span>' % f)
            continue
        rk = 1 + sum(1 for g, w in ex.items() if g != f and w < v)
        if f == boat:
            cells.append('<span style="border:2px solid %s;border-radius:5px;padding:0 4px;color:%s;font-weight:800">'
                         '%d号 %.2f<span style="font-size:10px">(%d位)</span></span>' % (_I_COLOR, _I_COLOR, f, v, rk))
        else:
            col = "#b23b3b" if v >= worst - 1e-9 else "#2c3742"
            cells.append('<span style="color:%s">%d号 %.2f<span style="font-size:10px;color:#8a97a5">(%d位)</span></span>'
                         % (col, f, v, rk))
    return '<span style="white-space:nowrap">' + "　".join(cells) + "</span>" + tail


def _ex6_cell(ex, hl=None, color=None, rc=None):
    """v114(2026-09-22 藤田指示「E F G I案にも 展示タイム 進入 本番進入 入れてほしい」):
    E/F/G案ブロック用の「展示タイム（1〜6号艇）」セル。最速=緑太字・最遅=赤。
    hl=強調する艇（E案は展示最速＝2着の軸）を color の枠で囲む。4艇未満なら『展示待ち』。"""
    try:
        hl = int(hl) if hl not in (None, "") else None
    except (TypeError, ValueError):
        hl = None
    ...
    if rc is not None:   # v128: 級・勝率・2連率つきの表
        _inf = _boat_info(rc)
        if _inf:
            return _boat6_table(ex, _inf, hl=hl, color=color)
    if len(ex) < 4:
        return '<span style="color:#8a6d1f;font-weight:700">&#9203; 展示待ち</span>'
    ...
    for f in range(1, 7):
        ...
        if v <= best + 1e-9:
            ...
            col = "color:#2c3742"
        box = ("border:2px solid %s;border-radius:5px;padding:0 4px;" % (color or "#1c7a38")) if f == hl else ""
        cells.append('<span style="%s%s">%d号 %.2f<span style="font-size:10px;color:#8a97a5">(%d位)</span></span>'
                     % (box, col, f, v, rk))
    return '<span style="white-space:nowrap">' + "　".join(cells[:3]) + "</span><br>" \
           '<span style="white-space:nowrap">' + "　".join(cells[3:]) + "</span>"


def _i_lane_html(ln, lr=None, settled=False):
    """v113(2026-09-21 藤田指示「I案に 進入 1-2-3-4-5-6 枠なり／本番進入 ＝展示どおり も追加して」):
    I案ブロックの展示セルの下に出す進入2行。カードの _lane_line/_lane_real_line と同じ言い方・色。
    ln = 展示進入 {"text","wakunari"}（未公開なら None）／lr = 本番進入(uv_json の lane_real)。"""
    out = ""
    if ln and ln.get("text"):
        if ln.get("wakunari"):
            out += ('<div style="font-size:11px;margin-top:2px">進入 <b style="color:#1c7a38">%s</b> '
                    '<span style="color:#1c7a38">枠なり</span></div>') % ln["text"]
        else:
            out += ('<div style="font-size:11px;margin-top:2px">進入 <b style="color:#b8860b">%s</b> '
                    '<span style="color:#b8860b">枠なり崩れ</span></div>') % ln["text"]
    else:
        out += '<div style="font-size:11px;margin-top:2px;color:#8a97a5">進入 展示待ち</div>'
    if lr and lr.get("known"):
        chg = lr.get("changed")
        if chg is None and ln and ln.get("text"):
            chg = (lr.get("text") != ln.get("text"))
        if chg:
            out += ('<div style="font-size:11px">本番進入 <b style="color:#b23b3b">%s</b> '
                    '<span style="color:#b23b3b">＝展示から変化</span></div>') % lr.get("text")
        else:
            out += ('<div style="font-size:11px">本番進入 <b>%s</b> '
                    '<span style="color:#8a97a5">＝展示どおり</span></div>') % lr.get("text")
    elif not settled:
        out += '<div style="font-size:11px;color:#8a97a5">本番進入 レース後に表示</div>'
    return out
...
# ============ v119(2026-09-22 藤田指示「値段の見方を直す＋モデルに場と風・波を足す。記録のみで出して」) ============
#   ① 判定時オッズを1割引いてEVを計算する（当たる目ほど締切までにオッズが縮む。9/20 判定時10〜150倍の的中82本で
#      払戻/判定時オッズ 中央値0.90・平均0.93、F/G実弾の的中3本は0.36〜0.71倍）。
#   ② 1号艇の1着確率を「場」と「風」で補正する（logit に加算）。data/hist 1〜9月・40,317R で当てはめ、
#      1〜5月で学習→6〜9月で検証して logloss 0.6291→0.6269（改善）。戸田−0.62・平和島−0.37・鳴門−0.27・江戸川−0.24・
#      桐生−0.15／大村+0.19・徳山+0.24・若松+0.28・芦屋+0.21・福岡+0.20、風5m以上−0.17・7m以上さらに−0.16。波は効かないので入れない。
#   E2/F2/D2案＝E/F/D案と同じ形で、確率と値段だけ上の2つで直したもの。**記録のみ**（実弾は今のE/F/D案のまま）。
_ODDS_SHRINK = 0.90
_P1_VENUE = {"1": -0.145, "2": -0.624, "3": -0.235, "4": -0.37, "5": 0.12, "6": -0.003, "7": -0.02, "8": 0.15, "9": 0.093, "10": -0.077, "11": -0.029, "12": 0.08, "13": 0.281, "14": -0.271, "15": -0.012, "16": -0.054, "17": 0.054, "18": 0.242, "19": 0.212, "20": 0.065, "21": 0.085, "22": 0.202, "23": 0.041, "24": 0.19}
_P1_W5, _P1_W7, _P1_C = -0.174, -0.158, -0.026


def _p1_delta(rc):
    """1号艇の1着確率の logit 補正量（場＋風）。取れなければ 0。"""
    try:
        jcd = int(rc.get("stadium_number") or rc.get("race_stadium_number") or 0)
    except (TypeError, ValueError, AttributeError):
        jcd = 0
    wind = None
    try:
        wind = float(((rc.get("preview") or {}).get("wind_speed")))
    except (TypeError, ValueError, AttributeError):
        wind = None
    d = _P1_C + float(_P1_VENUE.get(jcd, 0.0))
    if wind is not None:
        if wind >= 5:
            d += _P1_W5
        if wind >= 7:
...
def _tri_p1_adjust(tri, delta):
    """3連単確率 tri（キーは (i,j,k) または "i-j-k"）の1号艇1着の合計を logit で delta 動かし、全体を合計1のまま配り直す。"""
    if not tri or not delta:
        return dict(tri or {})
    def first(k):
        return int(k[0]) if isinstance(k, tuple) else int(str(k).split("-")[0])
    p1 = sum(v for k, v in tri.items() if first(k) == 1)
    tot = sum(tri.values())
    if tot <= 0 or p1 <= 0 or p1 >= tot:
        return dict(tri)
    q = p1 / tot
    z = float(np.log(q / (1 - q))) + delta
    q2 = 1.0 / (1.0 + float(np.exp(-z)))
    a, b = q2 / q, (1 - q2) / (1 - q)
    return {k: v * (a if first(k) == 1 else b) for k, v in tri.items()}


def formation_pick_v2(rc, boats, odds_map):
    """v119 E2案（記録のみ）: E案と同じ形・同じゲート。8点の平均EVだけ「補正確率×判定時オッズ×0.9」で測り直す。"""
    try:
        g = formation_pick(boats, odds_map)
    except Exception:
        return {"flag": False, "reason": "E案の計算に失敗"}
    if not g.get("picks"):
        return dict(g, flag=False)
    try:
        tri = _tri_p1_adjust(trifecta_probs_pl(predict_power(boats)), _p1_delta(rc))
    except Exception:
        return dict(g, flag=False, reason="補正確率の計算に失敗")
    picks = []; es = 0.0
    for p in g["picks"]:
        i, j, k = [int(x) for x in p["combo"].split("-")]
        pc = calib_p(tri.get((i, j, k), 0.0)); o = float(p["odds"]) * _ODDS_SHRINK
        e = pc * o; es += e
        picks.append(dict(p, ev2=round(e, 2)))
    ev2 = es / len(picks)
    out = dict(g, picks=picks, ev2=round(ev2, 2), delta=round(_p1_delta(rc), 3))
    if g.get("strong_skip") or (g.get("reason") and ("明らかに強い" in str(g.get("reason")))):
        return dict(out, flag=False)
    sb = _form_strong_block(boats, g.get("second") or [])
    if sb:
        return dict(out, flag=False, reason="%d号艇が明らかに強いのに2着候補の外（E案と同じ見送り）" % sb["frame"])
    if ev2 < _FORM_EV:
        return dict(out, flag=False, reason="補正後の8点平均EV %.2f が %.1f 未満" % (ev2, _FORM_EV))
    return dict(out, flag=True, reason="補正後の8点平均EV %.2f（場・風で1号艇を補正、判定時オッズ×0.9）" % ev2)


def f_pick_v2(rc, boats, odds_map):
    """v119 F2案（記録のみ）: F案と同じ作り。改良モデルの確率に場・風の補正をかけ、EVは判定時オッズ×0.9で測る。"""
    if not odds_map:
        return {"decision": "見送り", "picks": [], "total": 0, "reason": "オッズ未取得"}
    pm = f_model_probs(rc, boats)
    if not pm:
        return {"decision": "見送り", "picks": [], "total": 0, "reason": "改良モデルの計算に必要なデータが不足"}
    pm = _tri_p1_adjust(pm, _p1_delta(rc))
    od = {}
    for k in pm:
        try:
            od[k] = float(odds_map.get(k) or 0)
        except (TypeError, ValueError):
            od[k] = 0.0
    ...
    for k, e in ex.items():
        ...
        if not (_F_LO <= o <= _F_HI):
            continue
        pb = e / ze
        ev = pb * o * _ODDS_SHRINK
        if ev < _F_EV:
            continue
        cands.append({"combo": k, "Pb": round(pb, 5), "odds": o, "ev": round(ev, 3), "stake": _F_UNIT})
    cands.sort(key=lambda x: x["ev"], reverse=True)
    if not cands:
        return {"decision": "見送り", "picks": [], "total": 0, "reason": "補正後に合成EV≥1.00の組が無い"}
    return {"decision": "買い", "picks": cands, "total": _F_UNIT * len(cands),
            "reason": "場・風補正＋判定時オッズ×0.9で合成EV≥1.00の%d点（記録のみ）" % len(cands)}


def bestroi_pick_v2(rc, boats, odds_map):
    """v119 D2案（記録のみ）: D案（10〜15倍×較正EV≥1.2の上位3点）を、補正確率×判定時オッズ×0.9で測り直す。"""
    if not odds_map:
        return {"decision": "見送り", "picks": [], "total": 0, "reason": "オッズ未取得"}
    try:
        tri = _tri_p1_adjust(trifecta_probs_pl(predict_power(boats)), _p1_delta(rc))
    except Exception:
        return {"decision": "見送り", "picks": [], "total": 0, "reason": "確率の計算に失敗"}
    cands = []
    for ijk, p in tri.items():
        cs = f"{ijk[0]}-{ijk[1]}-{ijk[2]}"
        try:
            o = float(odds_map.get(cs) or 0)
        except (TypeError, ValueError):
            continue
        if not (_DROI_LO <= o <= _DROI_HI):
            continue
        ev = calib_p(p) * o * _ODDS_SHRINK
        if ev < _DROI_EV:
            continue
        cands.append({"combo": cs, "odds": o, "ev": round(ev, 2), "stake": _DROI_UNIT})
    cands.sort(key=lambda x: x["ev"], reverse=True)
    picks = cands[:_DROI_N]
    if not picks:
        return {"decision": "見送り", "picks": [], "total": 0, "reason": "補正後に条件を満たす組が無い"}
    return {"decision": "買い", "picks": picks, "total": _DROI_UNIT * len(picks),
            "reason": "場・風補正＋判定時オッズ×0.9で較正EV≥1.2の上位%d点（記録のみ）" % len(picks)}
...
def _v2_safe(fn, *a):
    try:
        ...
        print("[v119]", fn.__name__, _e, flush=True)
        return {"flag": False, "decision": "見送り", "picks": [], "reason": "計算エラー"}
...
_FORM_STRONG_GAP = 1.0   # v118: 2〜6号艇で全国勝率1位が2番手よりこれ以上高ければ「明らかな強者」


def _form_strong_block(boats, second):
    """v118(2026-09-22 藤田指示「E案に『2〜4号艇に明らかな強者がいて、それが2着候補に入っていなければ見送り』を足して」):
    2〜6号艇で全国勝率1位の艇が 2〜4号艇 にいて、A1 または 2番手より勝率が1.0以上高く、
    その艇が2着候補(展示最速とその内隣)に入っていなければ、その艇の番号と説明を返す。該当しなければ None。
    根拠(data/hist 1/1〜9/20・E案の形 10,521R・EV条件なし): 該当3,127R 的中9.8%・回収63.9% ／
    明らかな強者なし7,394R 的中18.3%・回収77.9%。強者が2着32.7%・2着か3着53.0%。発端=9/22福岡7R(4号A1→1-4-3)。"""
    try:
        B = {int(b["frame"]): b for b in boats}
        nat = {f: float(B[f].get("nat") or 0) for f in range(2, 7)}
    except Exception:
        return None
    top = max(range(2, 7), key=lambda f: nat[f])
    if top not in (2, 3, 4) or top in (second or []):
        return None
    sec = max(nat[f] for f in range(2, 7) if f != top)
    cls = str(B[top].get("cls") or "")
    if not (cls == "A1" or nat[top] - sec >= _FORM_STRONG_GAP):
        return None
    return {"frame": top, "cls": cls, "nat": round(nat[top], 2), "gap": round(nat[top] - sec, 2)}


def _form_watch(boats, second):
    """v120(2026-09-22 藤田指示「作成して」＝若松3Rを受けた見送り候補2つを記録のみで足す):
    E案の判定は変えずに、次の2つに当たるかを記録する（後で『このルールがあったら見送り』を比べるため）。
      watch_a  : 1号艇がA級 または 2〜6号艇にA級が4人以上（8/20〜9/20 EV込み: 1号A級20R 25.2%／A級4人以上12R 的中0）
      watch_loc: 2〜6号艇で当地勝率1位の艇が2〜4号艇にいて、A1 または 当地勝率が2番手より1.0以上高いのに2着候補の外
                 （若松3R: 2号艇A1・当地7.29が1位→1-2-5。全国勝率だけ見る v118 のルールでは拾えなかった）"""
    out = {"watch_a": False, "watch_loc": None, "nA": None, "cls1": None}
    try:
        B = {int(b["frame"]): b for b in boats}
        cls1 = str(B[1].get("cls") or "")
        nA = sum(1 for f in range(2, 7) if str(B[f].get("cls") or "") in ("A1", "A2"))
        out.update(cls1=cls1, nA=nA, watch_a=bool(cls1 in ("A1", "A2") or nA >= 4))
        loc = {f: float(B[f].get("loc") or 0) for f in range(2, 7)}
        top = max(range(2, 7), key=lambda f: loc[f])
        if top in (2, 3, 4) and top not in (second or []) and loc[top] > 0:
            sec = max(loc[f] for f in range(2, 7) if f != top)
            if str(B[top].get("cls") or "") == "A1" or loc[top] - sec >= 1.0:
                out["watch_loc"] = top
    except Exception:
        pass
    return out
...
def formation_pick(boats, odds_map):
    ...
    base.update(_form_watch(boats, second))   # v120: 記録のみの見送り候補
    if top != 1:
    ...
    # v118: 2〜4号艇に明らかな強者がいて2着候補に入っていなければ見送り（買い目・EVは記録に残す）
    _sb = _form_strong_block(boats, second)
    if _sb:
        return dict(base, flag=False, strong_skip=_sb["frame"],
                    reason=("%d号艇が明らかに強い（%s・勝率%.2f・2番手より+%.2f）のに2着候補(%s号艇)に入っていない＝"
                            "高オッズはその強さの裏返しなので見送り"
                            % (_sb["frame"], _sb["cls"] or "-", _sb["nat"], _sb["gap"],
...
# ==================== 締切5分前 最終判定カード（/final）====================
# v136: 本体はキャッシュ生成用に改名（ルートは下の final_page）
def _final_page_build():
    now = dt.datetime.now(dt.timezone(dt.timedelta(hours=9)))
    hd = now.strftime("%Y%m%d")
    ...
    def _ixplan_stats(k):
        """v123: I2/I3/I4案の当日集計。見送り＝I案が立たなかったレース。"""
        buy = hits = bset = skip = 0; stake = 0.0; ret = 0.0
        for r, u in uv_today:
            v = ((u or {}).get("ixplan") or {}).get(k) or {}
            if not v.get("picks"):
                if (u or {}).get("iplan") is not None:
                    skip += 1
                continue
            buy += 1
            if r.get("status") == "settled" and str(v.get("hit", "")) != "":
                bset += 1; stake += sum(fnum(p.get("stake")) for p in v["picks"])
    ...
    # v116(2026-09-22 藤田指示「実際に買いになってレースが終わるまではここの右側とかその案の下に出して。
    #   終わったらいつも通りに戻して」): 実弾枠の当日成績の右に、その案で買った未精算レースを出す。精算されたら消える。
    def _live_pending(plan):
        out = []
        for r, u in uv_today:
            if r.get("status") == "settled" or not u:
                continue
            items = []   # (買い目の文字, 金額)
            try:
                if plan == "A":
                    if u.get("dec_stable") == "買い" and fnum(u.get("total")) > 0 \
                            and (r.get("date"), str(r.get("jcd")), str(r.get("rno"))) not in _A_STOPPED:
                        items = [(p.get("combo"), p.get("odds"), fnum(p.get("stake"))) for p in (u.get("picks") or [])]
                elif plan == "B":   # v124: B案(≥40混戦)。買い目はA案と共通(足フィルタ後)
                    if u.get("dec40") == "買い" and fnum(u.get("total")) > 0:
                        items = [(p.get("combo"), p.get("odds"), fnum(p.get("stake"))) for p in (u.get("picks") or [])]
                elif plan == "E":
                    v = u.get("form") or {}
                    if v.get("flag"):
                        items = [(p.get("combo"), p.get("odds"), fnum(p.get("stake"))) for p in (v.get("picks") or [])]
                elif plan == "D":
                    items = [(p.get("combo"), p.get("odds"), fnum(p.get("stake"))) for p in ((u.get("droi") or {}).get("picks") or [])]
                elif plan == "F":
                    items = [(p.get("combo"), p.get("odds"), fnum(p.get("stake"))) for p in ((u.get("fplan") or {}).get("picks") or [])]
                elif plan == "G":
                    v = u.get("gplan") or {}
                    if v.get("status") == "買い":
                        items = [(p.get("combo"), p.get("odds"), fnum(p.get("stake"))) for p in (v.get("picks") or [])]
                    ...
                    v = u.get("iplan") or {}
                    if v.get("flag"):
                        items = [("単勝%s号艇" % v.get("boat"), None, float(_I_UNIT))]
                elif plan in ("I2", "I4"):   # v123 / v131: まとめて1行
                    v = (u.get("ixplan") or {}).get(plan) or {}
                    if v.get("picks"):
                        items = [("%s %s<span class=\"tsub\">(%d点)</span>" % (v["picks"][0].get("type"), _ix_fmt(plan, v), len(v["picks"])),
                                  None, float(sum(fnum(p.get("stake")) for p in v["picks"])))]
                elif plan in ("R10", "R20"):   # v117: 波乱の目(≤10%/≤20%)。展示で1号艇3位以下(ex_skip False)の時だけ買い
                    v = u.get("rare" if plan == "R10" else "rare20") or {}
                    if v.get("flag") and v.get("picks") and v.get("ex_skip") is False:
                        items = [("%s-[%s]-[全]" % (v.get("head"), "".join(str(x) for x in (v.get("second") or []))),
                                  None, float(sum(fnum(p.get("stake")) for p in v["picks"])))]
                        items[0] = (items[0][0] + '<span class="tsub">(%d点)</span>' % len(v["picks"]), None, items[0][2])
                elif plan in ("RB2", "RB4"):   # v126b: 波乱BOX
                    v0 = u.get("rare20") or {}
                    if v0.get("flag") and v0.get("ex_skip") is False and _box_ok(v0):   # v132
                        bx = v0.get("box2" if plan == "RB2" else "box4") or {}
                        if bx.get("picks"):
                            items = [("%s BOX %s<span class=\"tsub\">(%d点)</span>%s" % (bx.get("type"), "".join(str(x) for x in bx.get("boats", [])), len(bx["picks"]),
                                                                                      (' <b style="color:#8a6400">&#128081;A級</b>' if _box_zone(v0) == "A" else "")),
                                      None, float(sum(fnum(p.get("stake")) for p in bx["picks"])))]
            except Exception:
                items = []
            items = [it for it in items if it[0]]
            if items:
                out.append((str(r.get("closed_at") or ""), r, items))
        out.sort(key=lambda x: x[0])
        return out

    def _live_pending_html(plan, color):
        pl = _live_pending(plan)
        if not pl:
            return ""
        col = color or "#1c7a38"
        tot = sum(it[2] for _, _, its in pl for it in its)
        h = ('<div style="flex:1 1 380px;min-width:300px;background:#fff;border:2px solid %s;border-radius:12px;padding:8px 12px">'
             '<div style="font-weight:800;color:%s;font-size:13px">&#128994; 買ったレース（結果待ち） %d件・%s円</div>'
             '<table style="margin-top:4px;font-size:13px;width:100%%;table-layout:auto"><tr><th>締切</th><th>場R</th>'
             '<th style="text-align:left">買い目</th><th>金額</th><th>締切まで</th></tr>'
             % (col, col, len(pl), f"{int(tot):,}"))
        for ca, r, its in pl:
            body = " ".join('<span style="white-space:nowrap"><span class="combo" style="color:%s">%s</span>%s</span>'
                            % (col, c, ('<span class="tsub">(%s倍)</span>' % o) if o not in (None, "") else "")
                            for c, o, _ in its)
            try:
                _hh, _mm = [int(x) for x in ca.split(":")[:2]]
                _t = now.replace(hour=_hh, minute=_mm, second=0, microsecond=0)
                cd = '<span class="cd" data-t="%d" data-done="締切済・結果待ち" style="font-weight:700">—</span>' % int(_t.timestamp() * 1000)
            except Exception:
                cd = "—"
            h += ('<tr><td>%s</td><td style="white-space:nowrap"><b>%s%sR</b></td><td style="text-align:left;white-space:normal;line-height:1.6">%s</td><td>%s円</td><td>%s</td></tr>'
                  % (ca, ven(r.get("jcd")), r.get("rno"), body, f"{int(sum(it[2] for it in its)):,}", cd))
        return h + '</table></div>'

    def _rare_today_stats(field):
        """v117: 波乱の目の当日集計。買い＝展示で1号艇3位以下(ex_skip False)、見送り＝展示2位以内(ex_skip True)。"""
        buy = bset = hits = skip = 0; stake = ret = 0.0
        for r, u in uv_today:
            v = (u or {}).get(field) or {}
            if not (v.get("flag") and v.get("picks")):
                continue
            if v.get("ex_skip") is True:
                skip += 1; continue
            if v.get("ex_skip") is not False:
                continue
            buy += 1
            if r.get("status") == "settled":
                bset += 1
                stake += sum(fnum(p.get("stake")) for p in v["picks"])
                ret += fnum(v.get("ret"))
                hits += 1 if v.get("hit") else 0
        return {"buy": buy, "bset": bset, "hits": hits, "stake": stake, "ret": ret, "skip": skip,
                "roi": (ret / stake * 100) if stake else 0}

    def _rare_box_today_stats(box):
        """v126b: 波乱BOXの当日集計(≤20%＝≤10%を含む・展示3位以下が買い)。"""
        buy = bset = hits = skip = 0; stake = ret = 0.0
        for r, u in uv_today:
            v = (u or {}).get("rare20") or {}
            bx = v.get(box) or {}
            if not (v.get("flag") and bx.get("picks")):
                continue
            if v.get("ex_skip") is True or (v.get("ex_skip") is False and not _box_ok(v)):   # v132: A・Bどちらも無しは見送り
                skip += 1; continue
            if v.get("ex_skip") is not False:
                continue
            buy += 1
            if r.get("status") == "settled" and str(bx.get("hit", "")) != "":
                bset += 1
                stake += sum(fnum(p.get("stake")) for p in bx["picks"])
                ret += fnum(bx.get("ret")); hits += 1 if str(bx.get("hit")) == "1" else 0
        return {"buy": buy, "bset": bset, "hits": hits, "stake": stake, "ret": ret, "skip": skip,
                "roi": (ret / stake * 100) if stake else 0}

    def _rare_box_ab_stats(mode):
        """v132: 波乱BOX(2連単＋3連単の合計)を A・B の目印で分けた当日集計。mode='or'|'and'。
        目印が無い記録(v132より前)は数えない。見送り＝展示で見送り or 目印に当てはまらない。"""
        buy = bset = hits = skip = 0; stake = ret = 0.0
        for r, u in uv_today:
            v = (u or {}).get("rare20") or {}
            ab = v.get("ab")
            if not (v.get("flag") and ab):
                continue
            if mode in ("orA", "orB"):   # v135: AかB × 1号艇A級/B級(級が記録されたv135以降だけ)
                if not ab.get("c1"):
                    continue
                ok = bool(ab.get("A") or ab.get("B")) and ((ab.get("c1") in ("A1", "A2")) == (mode == "orA"))
            else:
                ok = (ab.get("A") or ab.get("B")) if mode == "or" else (ab.get("A") and ab.get("B"))
            if v.get("ex_skip") is not False or not ok:
                if v.get("ex_skip") is not None:
                    skip += 1
                continue
            bxs = [v.get("box2") or {}, v.get("box4") or {}]
            if not any(b.get("picks") for b in bxs):
                continue
            buy += 1
            if r.get("status") == "settled" and all(str(b.get("hit", "")) != "" for b in bxs if b.get("picks")):
                bset += 1
                stake += sum(fnum(p.get("stake")) for b in bxs for p in (b.get("picks") or []))
                ret += sum(fnum(b.get("ret")) for b in bxs)
                hits += 1 if any(str(b.get("hit")) == "1" for b in bxs) else 0
        return {"buy": buy, "bset": bset, "hits": hits, "stake": stake, "ret": ret, "skip": skip,
                "roi": (ret / stake * 100) if stake else 0}

    def _v2_stats(key, rs):
        """v119: E2/F2/D2案(記録のみ)の集計。rs=[(row, uv)]。買い＝E2はflag、F2/D2は買い目あり。"""
        buy = bset = hits = skip = 0; stake = ret = 0.0
        for r, u in rs:
            v = (u or {}).get(key)
            if not isinstance(v, dict) or not v:
                continue
            ok = bool(v.get("flag")) if key == "form2" else bool(v.get("picks"))
            if not ok:
                skip += 1; continue
            buy += 1
            if r.get("status") == "settled" and str(v.get("hit", "")) != "":
                bset += 1
                stake += sum(fnum(p.get("stake")) for p in v.get("picks") or [])
                ret += fnum(v.get("ret")); hits += 1 if str(v.get("hit")) == "1" else 0
        return {"buy": buy, "bset": bset, "hits": hits, "stake": stake, "ret": ret, "skip": skip,
                "roi": (ret / stake * 100) if stake else 0}

    def _form_watch_rows():
        """v120: E案の買いを『見送り候補に当たる／当たらない』で分けた当日＋通算（記録のみ・判定は不変）。"""
        try:
            def st(rs, pred):
                buy = bset = hits = 0; stake = ret = 0.0
                for r, u in rs:
                    v = (u or {}).get("form") or {}
                    if not v.get("flag") or "watch_a" not in v or not pred(v):
                        continue
                    buy += 1
                    if r.get("status") == "settled" and str(v.get("hit", "")) != "":
                        bset += 1; stake += sum(fnum(p.get("stake")) for p in v.get("picks") or [])
                        ret += fnum(v.get("ret")); hits += 1 if str(v.get("hit")) == "1" else 0
                return {"buy": buy, "bset": bset, "hits": hits, "stake": stake, "ret": ret, "skip": 0,
                        "roi": (ret / stake * 100) if stake else 0}
            uv_all = [(r, _uvj(r)) for r in rows]
            def cum(pred):
                s_ = st(uv_all, pred)
                return "通算 %d件・的中%d・回収率%.1f%%" % (s_["bset"], s_["hits"], s_["roi"])
            items = (
                ("E案の買いのうち「1号艇A級 または 2〜6号艇にA級4人以上」＝見送り候補A（もし見送っていたら浮いた分）",
                 lambda v: bool(v.get("watch_a"))),
                ("E案の買いのうち「当地勝率1位の強い2〜4号艇が2着候補の外」＝見送り候補B",
                 lambda v: bool(v.get("watch_loc"))),
                ("E案の買いのうち見送り候補A・Bのどちらにも当たらない＝残る買い",
                 lambda v: not v.get("watch_a") and not v.get("watch_loc")),
            )
            h = ('<div class="tsub" style="margin-top:12px;font-weight:800;color:%s">'
                 '&#128270; E案の見送り候補（v120・記録のみ／E案の買い判定は変えていない）</div>'
                 '<div class="tsub" style="font-size:11px">若松3R（9/22・1号艇A1・2〜6号艇にA級5人・2号艇が当地勝率1位→1-2-5で外れ）を受けて追加。'
                 '8/20〜9/20の最終オッズでは、E案の買いのうち1号艇A級は20R・回収25.2%%、2〜6号艇にA級4人以上は12R・的中0。'
                 '2週間ほど並べて、見送り候補の方が明らかに負けていれば実際の判定に入れる。</div>') % _FORM_COLOR
            for lab, pred in items:
                h += _uv_row(lab + '<span style="font-weight:400">　（%s）</span>' % cum(pred),
                             st(uv_today, pred), color=_FORM_COLOR)
            return h
        except Exception as _e:
            print("[v120 rows]", _e, flush=True)
            return ""

    def _g_filter_rows():
        """v121: G案の買いを ③お金の集中 と ②1号艇以外の頭 で分けた当日＋通算（記録のみ・G案の買い判定は不変）。"""
        try:
            def orig(v):
                """v122: 元のG案(絞り込み前)の買い目・当否。v122以前は実際の買いがそのまま元のG案。"""
                if v.get("g0_picks"):
                    return v["g0_picks"], v.get("g0_hit", ""), v.get("g0_ret")
                if v.get("status") == "買い" and v.get("picks"):
                    return v["picks"], v.get("hit", ""), v.get("ret")
                return None, "", 0
            def head_of(v):
                if "head" in v:
                    return int(v.get("head") or 0)
                pk0 = orig(v)[0] or [{}]
                try:
                    return int(str(pk0[0].get("combo", "0")).split("-")[0])
                except Exception:
                    return 0
            def st(rs, pred):
                buy = bset = hits = 0; stake = ret = 0.0
                for r, u in rs:
                    v = (u or {}).get("gplan") or {}
                    pk0, h0, r0 = orig(v)
                    if not pk0 or not pred(v):
                        continue
                    buy += 1
                    if r.get("status") == "settled" and str(h0) != "":
                        bset += 1; stake += sum(fnum(p.get("stake")) for p in pk0)
                        ret += fnum(r0); hits += 1 if str(h0) == "1" else 0
                return {"buy": buy, "bset": bset, "hits": hits, "stake": stake, "ret": ret, "skip": 0,
                        "roi": (ret / stake * 100) if stake else 0}
            uv_all = [(r, _uvj(r)) for r in rows]
            def cum(pred):
                s_ = st(uv_all, pred)
                return ("通算 %d件・的中%d・投資%s円・払戻%s円・回収率%.1f%%"
                        % (s_["bset"], s_["hits"], f"{int(s_['stake']):,}", f"{int(s_['ret']):,}", s_["roi"]))
            has = lambda v: "g_conc" in v
            items = (
                ("元のG案（絞り込み前・3割以上下落の最大1点すべて）", lambda v: True),
                ("③ お金の集中：1番下がった組と2番目の差が%dpt以上（v121から記録）" % int(100 * _G_CONC_GAP),
                 lambda v: has(v) and bool(v.get("g_conc"))),
                ("③の外れ：差が%dpt未満（もし見送っていたら浮いた分）" % int(100 * _G_CONC_GAP),
                 lambda v: has(v) and not v.get("g_conc")),
                ("② 1号艇以外が頭（9/21の実弾開始から通算）", lambda v: head_of(v) not in (0, 1)),
                ("②の外れ：1号艇が頭", lambda v: head_of(v) == 1),
                ("③かつ②（＝v122からの実際の買い）", lambda v: has(v) and bool(v.get("g_conc")) and head_of(v) not in (0, 1)),
            )
            h = ('<div class="tsub" style="margin-top:12px;font-weight:800;color:%s">'
                 '&#128270; G案の絞り込み ③・②（元のG案の買いを分けたもの・記録のみ／v122から実際に買うのは③かつ②）</div>'
                 '<div class="tsub" style="font-size:11px">事前登録した2条件。後知恵版G 1,898R（判定時→最終オッズ）では、'
                 'G全体111.8%%に対し ③集中 117.1%%（1,024R・的中84）、②1号艇以外の頭 160.8%%（224R・的中20）。'
                 '本番と同じ「判定時→締切約5分前」で測り直し、的中30本以上たまってから実際の判定に入れるか決める。</div>') % _G_COLOR
            for lab, pred in items:
                h += _uv_row(lab + '<span style="font-weight:400">　（%s）</span>' % cum(pred),
                             st(uv_today, pred), color=_G_COLOR)
            return h
        except Exception as _e:
            print("[v121 rows]", _e, flush=True)
            return ""

    def _v2_block():
        """v119: 補正版(E2/F2/D2)を当日と通算で並べる。元の案(E/F/D)の同じ期間と比べられるようにする。"""
        try:
            uv_all = [(r, _uvj(r)) for r in rows]
            since = min((r.get("date") for r, u in uv_all if (u or {}).get("form2") is not None), default=None)
            uv_since = [(r, u) for r, u in uv_all if since and r.get("date") >= since]
            def cum(key):
                s_ = _v2_stats(key, uv_since)
                return ("通算 %d件・的中%d・投資%s円・払戻%s円・回収率%.1f%%"
                        % (s_["bset"], s_["hits"], f"{int(s_['stake']):,}", f"{int(s_['ret']):,}", s_["roi"]))
            def cum_old(key):
                buy = hits = 0; st = rt = 0.0
                for r, u in uv_since:
                    v = (u or {}).get(key) or {}
                    ok = bool(v.get("flag")) if key == "form" else bool(v.get("picks"))
                    if ok and r.get("status") == "settled" and str(v.get("hit", "")) != "":
                        buy += 1; st += sum(fnum(p.get("stake")) for p in v.get("picks") or [])
                        rt += fnum(v.get("ret")); hits += 1 if str(v.get("hit")) == "1" else 0
                return "元の案の同じ期間 %d件・的中%d・回収率%.1f%%" % (buy, hits, (rt / st * 100) if st else 0)
            h = ('<div class="tsub" style="margin-top:12px;font-weight:800;color:#2c3742">'
                 '&#128295; 補正版（v119・記録のみ）：1号艇の1着確率を「場」と「風」で補正し、判定時オッズを1割引いてEVを測り直した版</div>'
                 '<div class="tsub" style="font-size:11px">判定時オッズは当たる目ほど締切までに縮む（9/20の的中82本で平均0.93倍）。'
                 '戸田・平和島・鳴門・江戸川・桐生と風5m以上では1号艇を下げ、大村・徳山・若松・芦屋・福岡では上げる。'
                 '形・点数・金額は元の案と同じ。元の案と並べて、補正で成績が良くなるかを確かめる。</div>')
            for key, okey, lab, col in (("form2", "form", "E2案：E案の補正版", _FORM_COLOR),
                                        ("fplan2", "fplan", "F2案：F案の補正版", _F_COLOR),
                                        ("droi2", "droi", "D2案：D案の補正版", None)):
                h += _uv_row(lab + '<span style="font-weight:400">　（%s／%s）</span>' % (cum(key), cum_old(okey)),
                             _v2_stats(key, uv_today), color=col)
            return h
        except Exception as _e:
            print("[v119 block]", _e, flush=True)
            return ""

    # v63: 「どれを実際に買うのか」が一目で分かるように、実弾2つと記録専用を分離表示する。
    def _uv_row(label, s, live=False, color=None, plan=None, tag=None):
        hr = (s["hits"] / s["bset"] * 100) if s["bset"] else 0
        badge = ('<span style="background:%s;color:#fff;border-radius:6px;padding:1px 7px;'
                 'font-size:11px;font-weight:800;margin-right:6px">%s</span>' % (color or "#1c7a38", tag)) if tag else ('<span style="background:%s;color:#fff;border-radius:6px;padding:1px 7px;'
        ...
        pend = _live_pending_html(plan, color) if (live and plan) else ""
        return ('<div class="tsub" style="margin-top:10px;font-weight:700;color:%s">%s%s 当日成績</div>'
                ...
                '<div class="tsub">回収率<br><b>%.1f%%</b></div>'
                '<div class="tsub">見送り<br><b>%d</b> レース</div>'
                '</div>%s</div>') % (col, badge, label, s["buy"], s["hits"], hr,
                             f"{int(s['stake']):,}", f"{int(s['ret']):,}", s["roi"], s["skip"], pend)
    uv_live = ('<div class="fcard" style="border-color:#1c7a38;background:#f4fbf6">'
               ...
               # v127(2026-09-23 藤田指示): 並び＝I案 → 希少・波乱の目 → 波乱の目・拡張版(＋波乱BOX) → E案 → 残り
               + (_uv_row("I案：展示で2番手より%.2f秒以上速い艇の単勝（1点×%d円）%s" % (_I_MARGIN, _I_UNIT, "" if _I_LIVE else "・記録のみ"), _iplan_stats(), live=_I_LIVE, color=_I_COLOR, plan="I") if _I_LIVE else "")
               # v123: I案の艇を1着にした2連単・3連単
               + "".join(_uv_row("%s：I案の艇（H）を1着にした %s（%s×%d円）" % (lab, desc, n_, _IX_UNIT),
                                 _ixplan_stats(k), live=True, color=_I_COLOR, plan=k)
                         for (k, lab, desc), n_ in zip(_IX_PLANS, ("2〜3点", "8点")) if _IX_LIVE)
               # v117(2026-09-22 藤田指示「実際に買う枠に 希少・波乱の目(≤10%) と 拡張版(≤20%) を両方出して」)
               + _uv_row("希少・波乱の目（1号艇モーター激弱 &le;10%＝約1/180の希少・16点フォメで買い／展示で1号艇3位以下のときだけ）",
                         _rare_today_stats("rare"), live=True, color="#b8860b", plan="R10")
               + _uv_row("波乱の目・拡張版（1号艇モーター &le;20%＝1日約8件・16点フォメで買い／展示で1号艇3位以下のときだけ）",
                         _rare_today_stats("rare20"), live=True, color="#8a6d1f", plan="R20")
               # v126b: 波乱BOX(≤20%・展示3位以下)。2連単BOX 1以外上位3艇(6点)／3連単BOX 1を含む上位4艇(24点)
               + ((_uv_row("波乱BOX①：2連単BOX（1号艇以外の総合力上位3艇・6点×1,000円／波乱の目≤20%かつ展示で1号艇3位以下）",
                           _rare_box_today_stats("box2"), live=True, color="#8a6d1f", plan="RB2")
                   + _uv_row("波乱BOX②：3連単BOX（1号艇を含む総合力上位4艇・24点×1,000円／波乱の目≤20%かつ展示で1号艇3位以下）",
                             _rare_box_today_stats("box4"), live=True, color="#8a6d1f", plan="RB4")
                   # v132: 目印A(風4m以上)・B(全国勝率のばらつき1.3以上)で分けたBOX 2種の合計成績(参考・買い判定は変えない)
                   + _uv_row("波乱BOX（2連単＋3連単）のうち「AかB」に当てはまるレース（A=風4m以上／B=6艇の全国勝率のばらつき1.3以上）",
                             _rare_box_ab_stats("or"), live=False, color="#1c7a38", tag="内訳")
                   + _uv_row("波乱BOX（2連単＋3連単）のうち「AとB両方」に当てはまるレース",
                             _rare_box_ab_stats("and"), live=False, color="#b23b3b", tag="内訳")
                   # v135: AかBを1号艇の級で分けた成績(v135以降の記録から)
                   + _uv_row("&#128081; A級勝負レース：波乱BOXのうち「AかB × 1号艇A1・A2級」（" + _ZONE_A_NOTE + "）",
                             _rare_box_ab_stats("orA"), live=False, color="#b8860b", tag="内訳")
                   + _uv_row("波乱BOXのうち「AかB × 1号艇B1・B2級」（比べる用）",
                             _rare_box_ab_stats("orB"), live=False, color="#5c6b7a", tag="内訳")) if _RARE_BOX_LIVE else "")
               + _uv_row("E案：フォーメーション 1着1号艇・2着=展示最速(%d号艇以上)とその内隣・3着全"
                         "（8点×%d円・板に一桁オッズ無し(最人気%.0f倍以上)かつ平均EV&ge;%.1f・2〜4号艇の明らかな強者が2着候補の外なら見送り）"
               ...
               + _uv_row("D案：10〜15倍×較正EV&ge;1.2 の較正EV上位3点（1点%d円＝最大%d円・検証中）"
                         % (_DROI_UNIT, _DROI_UNIT * _DROI_N), _droi_stats(), live=True, plan="D")
               + (_uv_row("G案（②③版）：締切前に売れた中オッズの1点（判定時10〜50倍→締切%g〜%g分前に%d%%以上下落した組のうち最大の1点×%d円・③2番目との差%dpt以上かつ②1号艇以外が頭のときだけ）%s" % (_G_HI, _G_LO, int(100 * _G_DROP), _G_UNIT, int(100 * _G_CONC_GAP), "" if _G_LIVE else "・記録のみ"), _gplan_stats(), live=_G_LIVE, color=_G_COLOR, plan="G") if _G_LIVE else "")
               ...
               + '<div class="tsub" style="margin-top:8px">'
                 + ('<b>2026-09-23からB案を実弾に戻し、F案は記録のみ</b>にしました。'
                    ...
                    + ('C案・D案・E案も並行してライブ検証中。' if _C_LIVE else
                       '<b>C案は2026-09-21から記録のみ</b>。D案・E案も並行してライブ検証中。')
                    if (_B_LIVE and not _F_LIVE) else
                    ('A案・B案（過小評価キー）・D案（低中オッズ較正EV）を並行してライブ検証中。'
                     'A案とB案は<b>同じ買い目</b>なので、両方に「買い」が出ても買うのは1回分。'
                     if _B_LIVE else
                     '<b>B案は記録のみ、F案（改良モデル×市場の合成EV）が実弾</b>です。'))
                 + 
                 ...
                 + '<br>&#9888;&#65039; <b>波乱≤10%／≤20%</b>は1回16,000円（16点×1,000円）で、'
                 '≤20%は1日10件以上出ることがあり<b>金額ではここが最大</b>になります（実測 9/18は17件・179,600円）。'
                 '該当レースと展示の判定は下の「波乱の目」ブロックを見てください。'
    ...
    uv_shadow = ('<div class="fcard" style="border-color:#dbe2ea">'
                 ...
                 + ("" if _G_LIVE else _uv_row("G案（②③版）：締切前に売れた中オッズの1点（判定時10〜50倍→締切%g〜%g分前に%d%%以上下落した組のうち最大の1点×%d円・③2番目との差%dpt以上かつ②1号艇以外が頭のときだけ）%s" % (_G_HI, _G_LO, int(100 * _G_DROP), _G_UNIT, int(100 * _G_CONC_GAP), "" if _G_LIVE else "・記録のみ"), _gplan_stats(), live=_G_LIVE, color=_G_COLOR))
    ...
    def _ex_line(r):
        ...
        if rc is not None and r.get("status") != "settled":
            ...
            if ml is not None and -2 <= ml <= _BI_WINDOW and (_ex_rank1(rc) is None or _lane_desc(rc) is None):
        ...
        if len(ex) < 4:
            out = ('<div style="margin-top:4px;font-size:13px"><span class="tsub">展示タイム</span>　'
                   ...
                   '<span class="tsub" style="font-size:11px">　締切30〜40分前ごろ判明（この画面は1分ごとに自動更新）</span></div>')
            _ex_cache[ck] = out
            return out
        best = min(ex.values()); worst = max(ex.values())
        cells = []
    ...
    def card(r):
        ...
        # v126b(2026-09-23 藤田指示「記録じゃなくて実践に出して」): BOX 2種は ≤20% かつ 展示3位以下で買い。
        _rbsrc = _rare20 or _rare10     # v126b: ≤20%(≤10%を含む)で展示3位以下なら買い
        _rbox = [(k, (_rbsrc.get(k) or {})) for k in ("box2", "box4")] if (_rbsrc and _RARE_BOX_LIVE and _box_ok(_rbsrc)) else []   # v132: A or B
        _rbox = [(k, v) for k, v in _rbox if v.get("picks")]
        ...
        _ix_c = (uv.get("ixplan") or {}) if (uv and _IX_LIVE) else {}   # v123: I2/I3/I4案
        _ixpk = [(lab, k, _ix_c.get(k)) for k, lab, _ in _IX_PLANS if (_ix_c.get(k) or {}).get("picks")]   # v131: 案ごとに1行
        ...
        _plans += [lab for k, lab, _ in _IX_PLANS if (_ix_c.get(k) or {}).get("picks")]
        if _rarebuy:
            _plans.append(_rarelab)
        _rzone = _box_zone(_rbsrc) if _rbsrc else None   # v135
        if _rbox:
            _plans.append("波乱BOX&#128081;A級勝負" if _rzone == "A" else "波乱BOX")
        isbuy = bool(_plans)
        ...
        if isbuy:
            ...
            for _xl, _xk, _xv in _ixpk:
                stk = sum(fnum(p.get("stake")) for p in _xv["picks"]); _tot += stk
                h += '<tr><td style="color:%s;font-weight:700">%s</td><td class="combo">%s %s <span class="tsub">(%d点)</span></td><td>—</td><td>%s円</td><td>%s</td></tr>' % (
                    _I_COLOR, _xl, _xv["picks"][0].get("type"), _ix_fmt(_xk, _xv), len(_xv["picks"]), f"{int(stk):,}", "オッズ次第")
            for p in _gpk:
                o = p.get("odds"); stk = fnum(p.get("stake")); _tot += stk
                pay = (fnum(o) * stk) if (o is not None and stk) else None
            ...
            for _bk, _bx in _rbox:
                _bp = _bx.get("picks", [])
                _bt = sum(fnum(p.get("stake")) for p in _bp); _tot += _bt
                _bfmt = "%s BOX %s" % (_bx.get("type"), "".join(str(x) for x in _bx.get("boats", [])))
                _bcell = "結果で確定"
                if r.get("status") == "settled" and r.get("win_combo") and str(_bx.get("hit", "")) != "":
                    _bcell = (('<span class="res-hit">的中 払戻%s円</span>' % f"{int(fnum(_bx.get('ret'))):,}")
                              if str(_bx.get("hit")) == "1" else "不的中")
                h += ('<tr%s><td>%s</td><td class="combo">%s (%d点)</td><td>—</td><td>%s円</td><td>%s</td></tr>'
                      % ((' style="background:#fff4cc"' if _rzone == "A" else ""),
                         ('波乱BOX<br><b style="color:#8a6400;font-size:11px">&#128081;A級勝負</b>' if _rzone == "A" else "波乱BOX"),
                         _bfmt, len(_bp), f"{int(_bt):,}", _bcell))
        ...
        if uv:
            ...
            h += ('<div style="background:#fff8e6;border:1px solid #f0e2b8;border-radius:10px;padding:8px 10px;margin-top:8px;font-size:13px">'
                  '%s%s%s%s%s%s%s%s</div>') % (badge, inner, _lane_line(uv), _lane_real_line(uv),
                                          ...
                                          _midev_line(uv) + _fplan_line(uv) + _g_line(uv) + _i_line(uv) + _ix_line(uv))
        ...
        if r.get("status") == "settled" and r.get("win_combo"):
            ...
            if uv:
                ...
                # v123: I2/I3/I4案
                ixd = uv.get("ixplan") or {}
                for _k, _xl, _ in _IX_PLANS:
                    _v = ixd.get(_k) or {}
                    if _IX_LIVE and _v.get("picks") and str(_v.get("hit", "")) != "":
                        parts.append(('<span class="res-hit">%s 的中 払戻%s円</span>' % (_xl, f"{int(fnum(_v.get('ret'))):,}"))
                                     if str(_v.get("hit")) == "1" else '<span class="res-miss">%s 不的中</span>' % _xl)
                # v108: G案
                gvd = uv.get("gplan") or {}
    ...
    def _uv_candidates():
        try:
            ...
            data = None
        if not data:
            return {"A": _pending_block("#1c7a38",
        ...
        for jcd in range(1, 25):
            for rno in range(1, 13):
                ...
                if (str(jcd), str(rno)) not in judged:
                    ...
                    try:
                        _ipb = extract_boats(rc)
                        _ip = i_pick(_ipb)
                        if _ip.get("flag"):
                            _ip = dict(_ip, ex6={int(b["frame"]): fnum(b.get("ex")) for b in _ipb})
                            try:
                                _ip["lane6"] = _lane_desc(rc)   # v113: 展示進入
                            except Exception:
                                _ip["lane6"] = None
                ...
                if _gv:
                    if _gv.get("second") and _gv.get("fast") and _gv.get("spread") is not None \
                            ...
                            and not _form_strong_block(_boats, _gv["second"]):   # v118
        ...
        # v114: E/F/G案ブロック共通の「展示タイム（1〜6号艇）＋展示進入＋本番進入」セル。
        #   展示が openapi に未反映で締切25分前〜2分後なら公式直前情報を直読み（カードの _ex_line と同じ経路）。
        def _exlane(jcd, rno, r=None, hl=None, color=None, closed=None):
            try:
                jcd = int(jcd); rno = int(rno)
                rc = find_race(data, jcd, rno)
            except Exception:
                rc = None
            settled = bool(r is not None and r.get("status") == "settled")
            ex = {}
            try:
                for b in (extract_boats(rc) if rc is not None else []):
                    v = fnum(b.get("ex"))
                    if v > 0:
                        ex[int(b["frame"])] = v
            except Exception:
            ...
            bi = None
            try:
                ld = _lane_desc(rc) if rc is not None else None
            except Exception:
                ld = None
            if not settled and (len(ex) < 4 or ld is None):
                try:
                    cl = closed
                    if cl is None and r is not None:
                        cl = _tri_parse_closed(r.get("closed_at"), now.date())
                    ml = ((cl - now).total_seconds() / 60.0) if cl else None
                    if ml is not None and -2 <= ml <= _BI_WINDOW:
                        bi = fetch_beforeinfo(hd, jcd, rno)
                except Exception:
                    bi = None
                if bi:
                    if len(ex) < 4 and len(bi.get("ex") or {}) >= 4:
                        ex = {}
                        for f, v in (bi.get("ex") or {}).items():
                            try:
                                ex[int(f)] = float(v)
                    ...
                    if ld is None:
                        try:
                            ...
                            ld = None
            u = _uvj_of(r) if r is not None else {}
            ln = u.get("lane") or {}
            ln = ln if (ln.get("known") and ln.get("text")) else ld
            lr = u.get("lane_real")
            if not lr and settled and rc is not None:
                try:
                    lr = _lane_real(rc, None, None)
                    if lr and ln and ln.get("text"):
                        lr["changed"] = (lr.get("text") != ln.get("text"))
                except Exception:
                    lr = None
            return ('<div style="text-align:left;white-space:nowrap"><div style="font-size:12px">%s</div>%s</div>'
                    % (_ex6_cell(ex, hl, color, rc=rc), _i_lane_html(ln, lr, settled=settled)))
        ...
        if ebuys:
            fh += ('<table style="margin-top:8px"><tr><th></th><th>締切</th><th>場R</th>'
                   '<th style="text-align:left">買い目（判定時オッズ）</th><th>各艇（級・全国勝率・全国2連率・展示）・進入</th><th>8点平均EV</th><th>金額</th><th>結果</th></tr>')
            for ca, r, gv in ebuys:
                ...
                fh += ('<tr class="buyrow" data-k="%s"><td><b style="color:#1c7a38">&#128994; 買い</b></td><td>%s</td><td><b>%s%sR</b></td>'
                       '<td style="text-align:left">%s</td><td>%s</td><td>%.2f</td><td>%s円</td><td>%s</td></tr>'
                       % ("E案|%s%sR|8点|締切%s" % (ven(r.get("jcd")), r.get("rno"), ca),
                          ca, ven(r.get("jcd")), r.get("rno"), body,
                          _exlane(r.get("jcd"), r.get("rno"), r, gv.get("fast"), _FORM_COLOR),
                          fnum(gv.get("ev1")),
        ...
        fh += ('<div class="tsub" style="margin-top:6px;font-size:12px">'
               ...
               'ここに出るのは形の条件（1号艇が総合力1位・展示最速が%d号艇以上・展示差&ge;%.2f・'
               '2〜4号艇に明らかな強者（A1または勝率が2番手より1.0以上高い）がいて2着候補に入っていない場合は除く）を満たしたレースで、'
        ...
        if fcands:
            fh += ('<div class="tsub" style="margin-top:8px;font-weight:700">未締切の候補（形の条件は満たした・締切5分前のオッズで買いか決まる）</div>'
                   ...
                   '<th>展示差</th><th>1号の総合力</th><th>各艇（級・全国勝率・全国2連率・展示）・進入</th>'
                   '<th style="text-align:left">買い目（8点×%s円・EV判定待ち）</th><th>状態</th></tr>'
                   % f"{_FORM_UNIT:,}")
            for c in fcands:
                cb = ["1-%d-%d" % (s, t) for s in c["second"] for t in range(2, 7) if t != s]
                fh += ('<tr><td>%s</td><td><b>%s%dR</b></td><td>%d号艇</td><td>%.2f</td><td>%s%%</td><td>%s</td>'
                    ...
                    c.get("model_p1"),
                    _exlane(c["jcd"], c["rno"], _today_by.get((str(c["jcd"]), str(c["rno"]))),
                            c["fast"], _FORM_COLOR, c["closed"]),
                    " ".join(cb), _e_status(c))
            fh += '</table>'
        else:
        ...
        try:
            ...
            if fbuys:
                gh += ('<table style="margin-top:8px"><tr><th></th><th>締切</th><th>場R</th>'
                       '<th style="text-align:left">買い目（判定時オッズ・合成EV）</th><th>各艇（級・全国勝率・全国2連率・展示）・進入</th><th>金額</th><th>結果</th></tr>')
                for ca, r, fv in fbuys:
                    ...
                    gh += ('<tr class="%s" data-k="%s"><td><b style="color:%s">&#128994; %s</b></td><td>%s</td><td><b>%s%sR</b></td><td style="text-align:left">%s</td><td>%s</td><td>%s円</td><td>%s</td></tr>'
                           % ("buyrow" if _F_LIVE else "", "F案|%s%sR|%s|締切%s" % (ven(r.get("jcd")), r.get("rno"),
                              " ".join(p.get("combo") for p in fv["picks"]), ca),
                              "#1c7a38" if _F_LIVE else "#8a97a5", "買い" if _F_LIVE else "記録", ca, ven(r.get("jcd")), r.get("rno"), body,
                              _exlane(r.get("jcd"), r.get("rno"), r), f"{int(amt):,}", resf))
                gh += '</table>'
            else:
        ...
        try:
            ...
            g8 += ('<div class="tsub" style="margin-top:6px;font-size:12px">'
                   ...
                   '無ければ見送り。<b>v122から</b>、さらに<b>③2番目に下がった組との差が%dpt以上</b>かつ<b>②1号艇以外が頭</b>のときだけ買います（外れたものは「元のG案なら買い」として記録のみ）。'
                   '<b>G案の買いは締切4〜6分前に決まる</b>ので、この表とカードで確認してください。</div>'
                   % (_G_HI, _G_LO, int(100 * _G_DROP), f"{_G_UNIT:,}", int(100 * _G_CONC_GAP)))
            if _gb:
                g8 += ('<table style="margin-top:8px"><tr><th></th><th>締切</th><th>場R</th><th>買い目</th>'
                       '<th>判定時→決定時</th><th>③集中・②頭<br><span style="font-weight:400">(v121 記録のみ)</span></th><th>各艇（級・全国勝率・全国2連率・展示）・進入</th><th>金額</th><th>結果</th></tr>')
                for ca, r, gv in _gb:
                    pk = (gv.get("picks") or [{}])[0]
                    if r.get("status") == "settled" and r.get("win_combo"):
                    ...
                    g8 += ('<tr class="%s" data-k="%s"><td><b style="color:#1c7a38">&#128994; 買い</b></td><td>%s</td><td><b>%s%sR</b></td>'
                           '<td><span class="combo" style="color:%s">%s</span></td><td>%s倍→%s倍（%d%%下落）</td><td style="font-size:11px">%s</td><td>%s</td><td>%s円</td><td>%s</td></tr>'
                           % ("buyrow" if _G_LIVE else "", "G案|%s%sR|%s|締切%s" % (ven(r.get("jcd")), r.get("rno"), pk.get("combo"), ca),
                              ...
                              round(-100 * fnum(pk.get("drop"))), (_g_filter_tag(gv).replace("[記録のみ] ", "").replace("）・<span", "）<br><span") or "—"),
                              _exlane(r.get("jcd"), r.get("rno"), r),
                              f"{int(fnum(pk.get('stake'))):,}", rs_))
                g8 += '</table>'
            if _gc:
                g8 += ('<div class="tsub" style="margin-top:8px;font-weight:700">候補（判定済み・決定待ち）</div>'
                       '<table style="margin-top:4px"><tr><th>締切</th><th>場R</th><th>監視中の組</th><th>各艇（級・全国勝率・全国2連率・展示）・進入</th><th>決定まで</th></tr>')
                for ca, r, gv in _gc:
                    try:
                        _hh, _mm = [int(x) for x in ca.split(":")[:2]]
                        ...
                        _cd = "—"
                    g8 += ('<tr><td>%s</td><td><b>%s%sR</b></td><td>10〜50倍 %d通り</td><td>%s</td><td>%s</td></tr>'
                           % (ca, ven(r.get("jcd")), r.get("rno"), len(gv.get("base") or {}),
                              _exlane(r.get("jcd"), r.get("rno"), r), _cd))
                g8 += '</table>'
            if _gs:
        ...
        try:
            ...
            def _i_lane_of(r):
                try:
                    _u = json.loads(r.get("uv_json") or "{}")
                except Exception:
                    _u = {}
                ln = _u.get("lane") or {}
                ln = ln if (ln.get("known") and ln.get("text")) else None
                if ln is None:
                    try:
                        _rc = find_race(data, int(r.get("jcd")), int(r.get("rno")))
                        ln = _lane_desc(_rc) if _rc is not None else None
                    except Exception:
                        ln = None
                return _i_lane_html(ln, _u.get("lane_real"), settled=(r.get("status") == "settled"))

            def _i_rc_of(jcd, rno):   # v128
                try:
                    return find_race(data, int(jcd), int(rno))
                except Exception:
                    return None

            def _i_ex_of(jcd, rno):
                try:
                    _rc = find_race(data, int(jcd), int(rno))
                    return {int(b["frame"]): fnum(b.get("ex")) for b in extract_boats(_rc)} if _rc is not None else {}
                except Exception:
                    return {}
            ...
            if _ib:
                i8 += ('<table style="margin-top:8px"><tr><th></th><th>締切</th><th>場R</th><th>買い目</th><th>各艇（級・全国勝率・全国2連率・展示）</th><th>金額</th><th>結果</th></tr>')
                for ca, r, iv in _ib:
                    if r.get("status") == "settled" and str(iv.get("hit", "")) != "":
                        ...
                        rs_ = '<span class="tsub">レース待ち</span>'
                    # v123: I2/I3/I4案の買い目と結果を同じ行に足す
                    try:
                        _xv = (json.loads(r.get("uv_json") or "{}").get("ixplan") or {}) if _IX_LIVE else {}
                    except Exception:
                        _xv = {}
                    _xc = ""; _xr = ""; _xamt = _I_UNIT
                    for _k, _xl, _ in _IX_PLANS:
                        _v = _xv.get(_k) or {}
                        if not _v.get("picks"):
                            continue
                        _xamt += sum(fnum(p.get("stake")) for p in _v["picks"])
                        _xc += '<div style="font-size:11px"><b>%s</b> %s %s</div>' % (_xl, _v["picks"][0].get("type"), _ix_fmt(_k, _v))
                        if str(_v.get("hit", "")) != "":
                            _xr += ('<div style="font-size:11px"><span class="res-hit">%s 的中 %s円</span></div>' % (_xl, f"{int(fnum(_v.get('ret'))):,}")
                                    if str(_v.get("hit")) == "1" else '<div style="font-size:11px"><span class="res-miss">%s 不的中</span></div>' % _xl)
                    i8 += ('<tr class="%s" data-k="%s"><td><b style="color:#1c7a38">&#128994; 買い</b></td><td>%s</td><td><b>%s%sR</b></td>'
                           '<td><span class="combo" style="color:%s">単勝 %s号艇</span>%s</td><td>%s</td><td>%s円</td><td>%s%s</td></tr>'
                           % ("buyrow" if _I_LIVE else "", "I案|%s%sR|単勝%s号艇|締切%s" % (ven(r.get("jcd")), r.get("rno"), iv.get("boat"), ca),
                              ca, ven(r.get("jcd")), r.get("rno"), _I_COLOR, iv.get("boat"), _xc,
                              _i_ex6_cell(_i_ex_of(r.get("jcd"), r.get("rno")) or {iv.get("boat"): iv.get("ex")},
                                          iv.get("boat"), iv.get("margin"), iv.get("course"),
                                          rc=_i_rc_of(r.get("jcd"), r.get("rno")))
                              + _i_lane_of(r),
                              f"{int(_xamt):,}", rs_, _xr))
                i8 += '</table>'
            if icands:
                i8 += ('<div class="tsub" style="margin-top:8px;font-weight:700">買い予定（展示で0.1秒以上速い・判定時に確定）</div>'
                       '<table style="margin-top:4px"><tr><th>締切</th><th>場R</th><th>買い目</th><th>各艇（級・全国勝率・全国2連率・展示）</th><th>確定まで</th></tr>')
                for c, j, rn, ip in icands:
                    ...
                    i8 += ('<tr><td>%s</td><td><b>%s%dR</b></td><td><span class="combo" style="color:%s">単勝 %s号艇</span></td>'
                           '<td>%s</td><td><span class="cd" data-t="%d">—</span></td></tr>'
                           % (c.strftime("%H:%M"), ven(j), rn, _I_COLOR, ip.get("boat"),
                              _i_ex6_cell(ip.get("ex6") or {ip.get("boat"): ip.get("ex")},
                                          ip.get("boat"), ip.get("margin"), ip.get("course"),
                                          rc=_i_rc_of(j, rn))
                              + _i_lane_html(ip.get("lane6")), _t))
                i8 += '</table>'
            ...
            i8 = ""
        return {"E": fh, "F": gh, "G": g8, "I": i8, "A": h}   # v127: 並び替えのため部品で返す
    # ---- 通算（全期間）の回収率と95%信頼区間 ----
    ...
    def _rare_block(field="rare", thr=10.0):
        ...
        data = _rare_data
        title = (("&#127775; 希少・波乱の目（1号艇モーター激弱 ≤10%＝約1/180の希少・16点フォメ＋2連単BOX＋3連単BOXで買い）" if _RARE_BOX_LIVE else "&#127775; 希少・波乱の目（1号艇モーター激弱 ≤10%＝約1/180の希少・16点フォメで買い）") if field == "rare" else
        ...
        for jcd in range(1, 25):
            for rno in range(1, 13):
                ...
                key = (str(jcd), str(rno))
                closed = _tri_parse_closed(_tri_closed_at(rc), now.date())
                if not done and closed is not None and (now - closed).total_seconds() >= 60:
                    # v129: openapi が遅れているときは公式の結果ページで先に当否を出す
                    _rcr = _race_with_result(rc, hd, jcd, rno)
                    if _rcr is not rc and _rcr is not None:
                        rc = _rcr
                        ex, tri = _payouts(rc)
                        done = (ex is not None or tri is not None)
        ...
        if n == 0:
            ...
            h += ('<table style="margin-top:8px"><tr><th>締切</th><th>場R</th><th>1号艇モ</th>'
                  '<th>各艇（級・全国勝率・全国2連率・展示）・進入</th><th>判定</th>'
                  '<th>%s</th><th>結果</th></tr>' % ('買い目（16点＋2連単BOX 6点＋3連単BOX 24点＝46,000円）'
                                                     if (_RARE_BOX_LIVE and field in ("rare", "rare20")) else '買い目(頭-2着4艇-3着全=16点=16,000円)'))
            for c in cands:
                rr = c["rr"]
                ...
                if _RARE_BOX_LIVE and field in ("rare", "rare20"):   # v126b: 波乱BOX 2種を併記 / v134: 希少(≤10%)にも表示
                    if not rr.get("ab"):
                        rr["ab"] = _box_ab(c["rc"])
                    elif "c1" not in rr["ab"]:   # v135: 前の記録には1号艇の級が無いので出走表から補う(表示のみ)
                        rr["ab"] = dict(rr["ab"], c1=_c1_cls(c["rc"]))
                    # v136: 台帳に記録があるレースは台帳のBOX(=実際に買った目)だけを出す。
                    #   台帳にBOXが無い＝BOX導入(v126・9/23 15時台)より前に判定したレース。画面で後から
                    #   組み直したBOXの的中を出すと「買っていない当たり」が成績に見えてしまうため出さない。
                    _ledu = (c["led"][1] if c["led"] else None)
                    _pre_box = bool(_ledu) and not any(((_ledu.get(k) or {}).get("picks")) for k in ("box2", "box4"))
                    c["pre_box"] = _pre_box
                    if _pre_box:
                        combos += ('<br><span class="tsub" style="color:#8a8a8a">＋BOX：このレースの判定時はBOX導入前'
                                   '（買っていない・成績に含めない）</span>')
                    for _bk in ("box2", "box4"):
                        if _pre_box:
                            break
                        _bx = ((_ledu or {}).get(_bk) if _ledu else None) or rr.get(_bk) or {}
                        if _bx.get("picks"):
                            combos += ('<br>＋%s BOX <b>%s</b> <span class="tsub">(%d点=%s円)</span>'
                                       % (_bx.get("type"), "".join(str(x) for x in _bx.get("boats", [])),
                                          len(_bx["picks"]), f"{len(_bx['picks']) * _RARE_UNIT:,}"))
                # v74: 展示タイムで見送り判定(公開前は「展示待ち」)
                bi = None
                mins_left = ((c["closed"] - now).total_seconds() / 60.0) if c["closed"] else None
                if (not c["done"]) and mins_left is not None and -2 <= mins_left <= _BI_WINDOW:
                ...
                # v115(2026-09-22 藤田指示「波乱の目・拡張版にも 展示タイム 進入 本番進入 入れてほしい」):
                #   1号艇の順位だけでなく 1〜6号艇の展示タイム＋展示進入＋本番進入を出す（E/F/G案 v114 と同じ見た目）。
                _ex6 = {}
                if bi and len(bi.get("ex") or {}) >= 4:
                    for _f, _v in (bi.get("ex") or {}).items():
                        try:
                            _ex6[int(_f)] = float(_v)
                        except (TypeError, ValueError):
                            pass
                else:
                    try:
                        for _b in extract_boats(c["rc"]):
                            if fnum(_b.get("ex")) > 0:
                                _ex6[int(_b["frame"])] = fnum(_b.get("ex"))
                    except Exception:
                        _ex6 = {}
                _ln6 = ld
                _lr6 = None
                if c["led"]:
                    try:
                        _u6 = json.loads(c["led"][0].get("uv_json") or "{}")
                    except Exception:
                        _u6 = {}
                    _l0 = _u6.get("lane") or {}
                    if _l0.get("known") and _l0.get("text"):
                        _ln6 = _l0
                    _lr6 = _u6.get("lane_real")
                if not _lr6 and c["done"]:
                    try:
                        _lr6 = _lane_real(c["rc"], None, None)
                        if _lr6 and _ln6 and _ln6.get("text"):
                            _lr6["changed"] = (_lr6.get("text") != _ln6.get("text"))
                    except Exception:
                        _lr6 = None
                ex6_s = ('<div style="text-align:left;white-space:nowrap"><div style="font-size:12px">%s</div>%s</div>'
                         % (_ex6_cell(_ex6, 1, color, rc=c["rc"]), _i_lane_html(_ln6, _lr6, settled=c["done"])))
                if ld is None:
                    lane_s = ''
                elif ld["wakunari"]:
                ...
                lane_s = ex6_s   # v115: 進入は6艇セルの下に出すので、1号艇の順位の後ろには付けない
                if exr is None:
                    ex_s = lane_s
                    jd = '<b style="color:#8a6d1f">&#9203; 展示待ち</b><br><span class="tsub" style="font-size:11px">締切30〜40分前ごろ判明（1分ごとに確認）</span>'
                    row_style = ''
                elif exr["rank"] <= _RARE_EX_SKIP_RANK:
                    ex_s = '<div style="text-align:left;font-size:12px">1号艇 <b>%s位</b>（%.2f）</div>' % (exr["rank"], exr["ex1"]) + lane_s
                ...
                if c["led"]:
                    ...
                    res = (('<span class="res-hit">的中 %s　払戻 %s円</span>' % (wc, f"{int(amt * _RARE_UNIT / 100):,}")) if hit_
                           else '<span class="res-miss">不的中(%s)</span>' % wc)
                elif c["done"]:
                    res = "締切後"
                if _RARE_BOX_LIVE and field in ("rare", "rare20") and c["done"] and not c.get("pre_box"):   # v126b: BOXの当否 / v134: 希少にも / v136: 導入前は出さない
                    _exd, _trd = _payouts(c["rc"])
                    _lb = (c["led"][1] if c["led"] else None) or {}
                    for _bk, _nm in (("box2", "2連単BOX"), ("box4", "3連単BOX")):
                        _bx = (_lb.get(_bk) if c["led"] else rr.get(_bk)) or {}   # v136: 台帳があれば台帳(実際に買った目)だけ
                        _src = _exd if _bx.get("type") == "2連単" else _trd
                        if not _bx.get("picks") or not _src:
                            continue
                        if any(p.get("combo") == _src[0] for p in _bx["picks"]):
                            res += '<br><span class="res-hit">%s 的中 %s　払戻 %s円</span>' % (_nm, _src[0], f"{int(_src[1] * _RARE_UNIT / 100):,}")
                        else:
                            res += '<br><span class="res-miss">%s 不的中</span>' % _nm
                both = (' <span style="color:#b8860b;font-size:11px">≤10%も該当</span>'
                        if (field != "rare" and rr["mot1"] <= 10.0) else '')
                if _RARE_BOX_LIVE and field in ("rare", "rare20"):   # v133: 判定(買い候補/見送り)の上に A・B の目印を大きく出す / v134: 希少にも
                    jd = _ab_box_html(rr) + jd
                    # v135: AかB × 1号艇A級は行ごと金色に(見送り=展示1・2位のときは変えない)
                    if _box_zone(rr) == "A" and not (exr and exr["rank"] <= _RARE_EX_SKIP_RANK):
                        row_style = ' style="background:#fff4cc;box-shadow:inset 6px 0 0 #d4a017"'
                        combos = ('<div style="color:#8a6400;font-weight:800;font-size:12px;margin-bottom:2px">&#128081; A級勝負レース：BOXを買う</div>'
                                  + combos)
        ...
        if field == "rare":
            ...
            h += ('<div class="tsub" style="margin-top:6px">※v77(9/18)フォーメーションで実際に買う枠（藤田指示）。'
                  ...
                  '通算は /api/tri/summary の rare_upset で自動集計。</div>'
                  + ('<div class="tsub" style="margin-top:6px">※v134(9/24 藤田指示): 波乱の目・拡張版と同じロジックで'
                     '＋2連単BOX（1号艇以外の総合力上位3艇・6点）＋3連単BOX（1号艇を含む上位4艇・24点）を併記。'
                     '≤10%は≤20%にも入るので<b>BOXは拡張版と同じ1回分だけ買う（二重には買わない）</b>。通算は rare_box2 / rare_box4。</div>'
                     if _RARE_BOX_LIVE else '') + '</div>')
        else:
    ...
    # v127(2026-09-23 藤田指示): 下の枠も I案 → 希少・波乱の目 → 波乱の目・拡張版 → E案 → 残り(F/G/A/C) → 各レース
    return HTMLResponse(head + bar + _NOTIFY_BAR + uv_live
                        + cand_block["I"] + rare10_block + rare_block + cand_block["E"]
                        + cand_block["F"] + cand_block["G"] + cand_block["A"] + midev_today
                        + body + cum_block + midev_block + uv_shadow
...
# ==================== v136(2026-09-26): /final を「作り置き」で即返す ====================
# 背景: /final は台帳全体(約4,300行)の uv_json を十数回読み直すため、Render上で1回の生成に約39秒。
#   さらに60秒ごとの自動更新でそのたびに39秒待ち → iPhone(Safari)で「開くまでに止まる／開けない」。
# 対策: 生成済みHTMLを保持して即返す。古くなっていたら裏で作り直す(見ている人がいる時だけ)。
#   買い目・判定ロジックは一切変更なし。表示の作り方だけ。
import threading as _thr_final
_FINAL_CACHE = {"body": None, "ts": 0.0}
_FINAL_LOCK = _thr_final.Lock()
_FINAL_MAX_AGE = 50   # 秒。これより古ければ裏で作り直す


def _final_rebuild(wait=False):
    if not _FINAL_LOCK.acquire(blocking=wait):
        return
    try:
        resp = _final_page_build()
        body = resp.body if hasattr(resp, "body") else str(resp).encode("utf-8")
        _FINAL_CACHE["body"] = body
        _FINAL_CACHE["ts"] = time.time()
    except Exception as e:
        print("[final-cache] 生成失敗: %r" % (e,), flush=True)
    finally:
        _FINAL_LOCK.release()


def _final_rebuild_bg():
    _thr_final.Thread(target=_final_rebuild, daemon=True).start()


@app.get("/final", response_class=HTMLResponse)
def final_page(fresh: int = 0):
    if _FINAL_CACHE["body"] is None or fresh:
        _final_rebuild(wait=True)          # 初回・?fresh=1 のときだけその場で作る
    elif time.time() - _FINAL_CACHE["ts"] > _FINAL_MAX_AGE:
        _final_rebuild_bg()                # 今は手元の版を即返し、次の更新で新しい版になる
    body = _FINAL_CACHE["body"]
    if body is None:
        return HTMLResponse("<p>生成中です。数十秒後に再読み込みしてください。</p>", status_code=503)
    age = int(time.time() - _FINAL_CACHE["ts"])
    gen = dt.datetime.fromtimestamp(_FINAL_CACHE["ts"], dt.timezone(dt.timedelta(hours=9))).strftime("%H:%M:%S")
    note = ('<div style="font-size:11px;color:#888;padding:2px 6px">表示データ生成 %s（%d秒前）'
            '・最新にするなら <a href="/final?fresh=1">今すぐ作り直す</a>（約40秒）</div>' % (gen, age)).encode("utf-8")
    body = body.replace(b"<body>", b"<body>" + note, 1)
    return HTMLResponse(body)


@app.on_event("startup")
def _final_warm():
    _final_rebuild_bg()   # 起動直後に1回作っておく(初回アクセスの待ちをなくす)
...
_RARE_UNIT = 1000
_RARE_BOX_LIVE = True   # v126: 波乱BOX(≤10%・展示3位以下)を買いとして表示する
...
# v130(2026-09-23 藤田指摘「買いか見送るか出ない」): 展示は締切の30〜40分前に出るのに、公式の直前情報を
#   読みに行くのが締切25分前からだった。45分前から読むようにして、展示が出たらすぐ買い/見送りを出す。
_BI_WINDOW = 45
...
# v132(2026-09-23 藤田指示): 波乱BOXの目印 A・B を付け、「AかB」「AとB両方」の成績を別に表示する。
#   買い判定は絞らない(_BOX_AB_GATE=False)。Trueにすると「AかB」のときだけBOXを買いにする。
#   A: 風速4m以上（直前情報）／B: 6艇の全国勝率のばらつき(標準偏差)が1.3以上。
#   検証(2026/1/1〜9/23・波乱の目≤20%かつ展示3位以下 1,174R): A or B 588R 102.9%(大穴3本抜き92.6%・前104/後102)、
#   どちらでもない 510R 62.1%。当てはまらないレースのBOXは記録のみ。
_BOX_AB_GATE = False
_BOX_AB_WIND = 4
_BOX_AB_NATSD = 1.3
...
def _box_ab(rc):
    """{"wind","natsd","A","B","ok"}。風が未公開なら A=False(勝率のばらつきBだけで判定)。"""
    w = None; sd = None
    try:
        pv = (rc or {}).get("preview") or {}
        if pv.get("wind_speed") is not None:
            w = float(pv.get("wind_speed"))
    except Exception:
        w = None
    try:
        ents = []
        _collect_with_key(rc, "national_win_rate", ents)
        v = [float(e.get("national_win_rate")) for e in ents if e.get("national_win_rate") is not None]
        if len(v) >= 5:
            sd = float(np.std(v))
    except Exception:
        sd = None
    A = bool(w is not None and w >= _BOX_AB_WIND)
    B = bool(sd is not None and sd >= _BOX_AB_NATSD)
    return {"wind": w, "natsd": (round(sd, 2) if sd is not None else None), "A": A, "B": B, "ok": bool(A or B),
            "c1": _c1_cls(rc)}


def _c1_cls(rc):
    """v135: 1号艇の級(A1/A2/B1/B2)。取れなければ None。"""
    try:
        for b in extract_boats(rc):
            if int(b.get("frame") or 0) == 1:
                return b.get("cls") or None
    except Exception:
        pass
    return None


# v135(2026-09-24 藤田指示「A級の時にはもっとわかりやすく表示」):
#   検証(2026/1/1〜9/23・波乱の目≤20%かつ展示で1号艇3位以下・BOX2種30,000円/R):
#     AかB × 1号艇A級(A1/A2) 288R 回収率121.8%(大穴3本抜き100.9%・前半122/後半121)
#     AかB × 1号艇B級       300R 回収率 84.7%(74.1%)
#   1号艇がA級だとモーターが弱くても人気が1号艇に集まり、外れた時の配当が大きい。
#   買い判定は変えない(AかBで買う)。画面で目立たせ、A級/B級の成績を分けて記録する。
_ZONE_A_NOTE = "過去288R 回収率121.8%（B級は84.7%）"


def _box_zone(rr):
    """v135: 'A'=AかB×1号艇A級 / 'B'=AかB×1号艇B級 / 'N'=A・Bなし / None=目印なし・級不明。"""
    ab = (rr or {}).get("ab") or {}
    if not ab:
        return None
    if not (ab.get("A") or ab.get("B")):
        return "N"
    c1 = ab.get("c1")
    if not c1:
        return None
    return "A" if c1 in ("A1", "A2") else "B"


def _box_ok(rr):
    """このレースの波乱BOXを買うか。v132より前の記録(ab無し)は当時どおり買い扱い。"""
    if not _BOX_AB_GATE:
        return True
    ab = (rr or {}).get("ab")
    return True if not ab else bool(ab.get("ok"))
...
def _ab_label(rr):
    ...
    parts = []
    if ab.get("A"):
        parts.append("A:風%sm" % (int(ab["wind"]) if ab.get("wind") is not None else "?"))
    if ab.get("B"):
        parts.append("B:勝率差%.2f" % ab["natsd"])
    if ab.get("A") and ab.get("B"):
        return ('<span style="background:#b23b3b;color:#fff;border-radius:5px;padding:0 5px;font-size:11px;font-weight:700">'
                '&#11088; AとB両方 %s</span>' % "・".join(parts))
    if parts:
        return '<span style="background:#1c7a38;color:#fff;border-radius:5px;padding:0 5px;font-size:11px;font-weight:700">AかB %s</span>' % "・".join(parts)
    return ('<span style="background:#e8ecf1;color:#5c6b7a;border-radius:5px;padding:0 5px;font-size:11px">'
            'A・Bなし（風%sm・勝率差%s）</span>' % (
                (int(ab["wind"]) if ab.get("wind") is not None else "?"),
                ("%.2f" % ab["natsd"]) if ab.get("natsd") is not None else "?"))

def _ab_box_html(rr):
    """v133(2026-09-23 藤田指示「買い候補の上に分かりやすく」): 判定欄の上に出す A・B の目印ボックス。"""
    ab = (rr or {}).get("ab") or {}
    if not ab:
        return ""
    w = ab.get("wind"); sd = ab.get("natsd")
    ws = ("%dm" % int(w)) if w is not None else "—"
    ss = ("%.2f" % sd) if sd is not None else "—"
    a_on, b_on = bool(ab.get("A")), bool(ab.get("B"))
    _zn = _box_zone(rr)
    _c1 = ab.get("c1")
    if _zn == "A":
        _zone_html = ('<div style="display:inline-block;text-align:center;background:#d4a017;color:#fff;border-radius:8px;'
                      'padding:4px 10px;margin-bottom:4px;min-width:150px;box-shadow:0 0 0 3px #f7e3a1">'
                      '<div style="font-weight:900;font-size:15px">&#128081; A級勝負レース</div>'
                      '<div style="font-size:11px">AかB × 1号艇%s</div>'
                      '<div style="font-size:10px;opacity:.9">%s</div></div><br>' % (_c1, _ZONE_A_NOTE))
    elif _zn == "B":
        _zone_html = ('<div style="display:inline-block;font-size:11px;color:#5c6b7a;background:#eef1f4;border-radius:6px;'
                      'padding:1px 8px;margin-bottom:4px">AかB × 1号艇%s（B級・過去84.7%%）</div><br>' % _c1)
    else:
        _zone_html = ""
    if a_on and b_on:
        head, bg, bd, fg = "&#11088; AとB両方", "#fdecec", "#b23b3b", "#b23b3b"
    elif a_on or b_on:
        head, bg, bd, fg = "AかB（勝負）", "#eaf6ee", "#1c7a38", "#1c7a38"
    else:
        head, bg, bd, fg = "A・Bなし", "#f4f6f8", "#c9d2dc", "#8a97a5"
    def chip(on, text):
        return ('<span style="display:inline-block;margin:1px 2px;padding:0 5px;border-radius:4px;font-size:11px;%s">%s</span>'
                % ("background:%s;color:#fff;font-weight:700" % fg if on else "background:#fff;color:#8a97a5;border:1px solid #dbe2ea", text))
    return _zone_html + ('<div style="display:inline-block;text-align:center;border:2px solid %s;background:%s;border-radius:8px;'
            'padding:3px 6px;margin-bottom:6px;min-width:140px">'
            '<div style="font-weight:800;color:%s;font-size:13px">%s</div>'
            '<div>%s%s</div></div><br>'
            % (bd, bg, fg, head, chip(a_on, "A 風%s" % ws), chip(b_on, "B 勝率差%s" % ss)
               + (('<span style="display:inline-block;margin:1px 2px;padding:0 5px;border-radius:4px;font-size:11px;%s">1号艇%s</span>'
                   % ("background:#d4a017;color:#fff;font-weight:700" if _zn == "A"
                      else "background:#fff;color:#8a97a5;border:1px solid #dbe2ea", _c1)) if _c1 else "")))
...
def _rare_upset_pick(rc, thr=10.0):
    ...
    try:
        ...
        pass
    # v126(2026-09-23 藤田指示): BOX 2種を追加。順位はモデル総合力(predict_power)、失敗時はモーター順。
    #   検証(2026年3〜9月・1号艇モーター≤10%かつ展示3位以下・100R): 2連単BOX 1以外上位3艇(6点) 的中33%・回収率114.6%、
    #   3連単BOX 1を含む上位4艇(24点) 的中60%・回収率118.0%。ただし≤20%(965R)では 86.1%/82.7%＝100%未満。
    #   100Rで10通り試した上位なので偶然の可能性あり → 前向きの実績で確かめる枠。
    _rank = (sorted(range(1, 7), key=lambda f: -_pw.get(f, 0.0)) if _pw
             else [1] + sorted(range(2, 7), key=lambda f: -m(f)))
    _top3x1 = [f for f in _rank if f != 1][:3]
    _top4 = _rank[:4]
    box2 = {"label": "2連単BOX(1以外上位3艇)", "type": "2連単", "boats": _top3x1,
            "picks": [{"combo": f"{a}-{b}", "stake": _RARE_UNIT, "type": "2連単"}
                      for a in _top3x1 for b in _top3x1 if a != b]}
    box4 = {"label": "3連単BOX(1含む上位4艇)", "type": "3連単", "boats": _top4,
            "picks": [{"combo": f"{a}-{b}-{c}", "stake": _RARE_UNIT, "type": "3連単"}
                      for a in _top4 for b in _top4 for c in _top4 if len({a, b, c}) == 3]}
    ...
    return {"flag": True, "mot1": round(mot1, 1), "head": head, "head_mot": head_mot,
            ...
            "box2": box2, "box4": box4, "ab": _box_ab(rc)}
...
def tri_judge_cycle():
    ...
    with _tri_lock:
            ...
            for rno in range(1, 13):
                ...
                try:
                    ...
                    uv_json = json.dumps({"decision": uv["decision"],
                                          ...
                                          "iplan": i_pick(boats),
                                          "ixplan": ix_pick(boats, i_pick(boats)),   # v123
                                          "h2": h2_pick(uv, odds_map),
                                          "form": formation_pick(boats, odds_map),
                                          "form2": _v2_safe(formation_pick_v2, rc, boats, odds_map),   # v119 記録のみ
                                          "fplan2": _v2_safe(f_pick_v2, rc, boats, odds_map),          # v119 記録のみ
                                          "droi2": _v2_safe(bestroi_pick_v2, rc, boats, odds_map),     # v119 記録のみ
        ...
        # v131: 今日の旧形式 I2/I3 記録を「I2案」1つにまとめて表示する(買い目・当否は変えない。I4は旧4点のまま)
        for r in rows:
            if r.get("date") != hd or '"ixplan"' not in (r.get("uv_json") or ""):
                continue
            try:
                _u9 = json.loads(r.get("uv_json") or "{}")
            except Exception:
                continue
            _x9 = _u9.get("ixplan") or {}
            if not _x9 or _x9.get("v") == 2:
                continue
            _n9 = _ix_upgrade(_x9)
            if _n9 is _x9:
                continue
            _u9["ixplan"] = _n9   # 表示をまとめるだけ。買い目・当否・status は変えない
            r["uv_json"] = json.dumps(_u9, ensure_ascii=False)
        # 精算（確定レース）
        for r in rows:
            if r.get("date") != hd:
                continue
            if r.get("status") == "settled":
                # v129: 公式ページで先に精算したレースは、openapi に結果が載ったら突き合わせる
                if '"_settle_src": "official"' not in (r.get("uv_json") or ""):
                    continue
                _rc0 = find_race(data, int(r["jcd"]), int(r["rno"]))
                _, _t0 = _payouts(_rc0) if _rc0 is not None else (None, None)
                if _t0 is None:
                    continue
                if str(_t0[0]) == str(r.get("win_combo")):
                    try:
                        _u0 = json.loads(r.get("uv_json") or "{}")
                        _u0["_settle_src"] = "official_ok"
                        try:
                            _lr0 = _lane_real(_rc0, (_u0.get("key") or {}).get("frame"), _u0.get("honmei"))
                            if _lr0:
                                _ln0 = _u0.get("lane") or {}
                                _lr0["changed"] = bool(_ln0.get("known") and _ln0.get("text") and _ln0["text"] != _lr0["text"])
                                _u0["lane_real"] = _lr0
                        except Exception:
                            pass
                        r["uv_json"] = json.dumps(_u0, ensure_ascii=False)
                    except Exception:
                        pass
                    continue
                print("[settle] 公式ページとopenapiの結果が違うため精算し直す", r.get("jcd"), r.get("rno"),
                      r.get("win_combo"), _t0[0], flush=True)
                r["status"] = "pending"
            rc = find_race(data, int(r["jcd"]), int(r["rno"]))
            _src_official = False
            if rc is None or _payouts(rc)[1] is None:
                # v129: 締切から1分過ぎても openapi に結果が無ければ公式の結果ページを読む
                _cl = _tri_parse_closed(r.get("closed_at"), now.date())
                if _cl is not None and (now - _cl).total_seconds() >= 60:
                    rc2 = _race_with_result(rc, hd, int(r["jcd"]), int(r["rno"]))
                    if rc2 is not None and (rc2.get("result") or {}).get("_src") == "official":
                        rc = rc2; _src_official = True
            if rc is None:
                continue
            _, tri = _payouts(rc)
            if tri is None:
            ...
            for _rf in ("rare", "rare20"):      # v70: ≤20%拡張版も同じ方法で精算
                ...
                if rr.get("flag") and rr.get("picks"):
                    ...
                    rr["hit"] = rhit; rr["ret"] = int(rret)
                    # v126: BOX 2種の精算(2連単は2連単の払戻、3連単は3連単の払戻)
                    _exp6, _ = _payouts(rc)
                    for _bk in ("box2", "box4"):
                        bx = rr.get(_bk) or {}
                        if not bx.get("picks"):
                            continue
                        src = _exp6 if bx.get("type") == "2連単" else (win_combo, pay)
                        if src is None:
                            continue
                        bh = 0; bret = 0.0
                        for p in bx["picks"]:
                            if p.get("combo") == src[0]:
                                bh = 1; bret += (p.get("stake", 0) or 0) * (float(src[1]) / 100.0)
                        bx["hit"] = bh; bx["ret"] = int(bret); rr[_bk] = bx
                    uv[_rf] = rr
            # 中オッズEV(記録のみ)の精算
            mv = uv.get("midev") or {}
            if mv.get("picks"):
            ...
            # v119: E2/F2/D2案(記録のみ)の精算。買い目があれば当否を付ける(E2は見送りでも比較用に付ける)。
            for _k2 in ("form2", "fplan2", "droi2"):
                v2 = uv.get(_k2) or {}
                if v2.get("picks"):
                    h2_ = 0; r2_ = 0.0
                    for p in v2["picks"]:
                        if p.get("combo") == win_combo:
                            h2_ = 1; r2_ += (p.get("stake", 0) or 0) * (pay / 100.0)
                    v2["hit"] = h2_; v2["ret"] = int(r2_); uv[_k2] = v2
            # D案(並行検証)の精算
            dv = uv.get("droi") or {}
            if dv.get("picks"):
                dhit = 0; dret = 0.0
            ...
            if iv0.get("flag"):
                ...
                if wp is not None:
                    ...
                    uv["iplan"] = iv0
            # v123: I2/I3/I4(I案の艇を1着にした2連単・3連単)の精算
            ix0 = uv.get("ixplan") or {}
            if ix0:
                _exp, _trp = _payouts(rc)
                uv["ixplan"] = _ix_settle(ix0, _exp, _trp)
            ...
            if gv8.get("g0_picks"):   # v122: 元のG案(記録のみ)の精算
                g0h = 0; g0r = 0.0
                for p in gv8["g0_picks"]:
                    if p.get("combo") == win_combo:
                        g0h = 1; g0r += (p.get("stake", 0) or 0) * (pay / 100.0)
                gv8["g0_hit"] = g0h; gv8["g0_ret"] = int(g0r)
            if gv8:
                uv["gplan"] = gv8
...
# v137(2026-09-24 藤田申告「これは実際に買えてたから台帳に記録して」):
#   9/23 の三国2R・浜名湖8R は、台帳にBOXが記録される前(v126導入前)に判定したが、BOXも実際に買っていた。
#   台帳の rare20 に box2/box4 を書き足して精算する(1点1,000円)。ほかの項目(16点フォメ・展示判定)は変えない。
#   すでに box2/box4 がある行はさわらない(何度動いても同じ結果)。書き足した行には manual_box を付ける。
#   (date, jcd, rno)。追加するときはここに足すだけ。
_MANUAL_BOX_FILL = [("20260923", 10, 2), ("20260923", 6, 8)]
_manual_box_done = False


def _tri_manual_box_fill():
    global _manual_box_done
    if _manual_box_done or not _MANUAL_BOX_FILL:
        return
    with _tri_lock:
        rows = tri_load_ledger()
        if not rows:
        ...
        for hd, jcd, rno in _MANUAL_BOX_FILL:
            for r in rows:
                if not (str(r.get("date")) == hd and str(r.get("jcd")) == str(jcd) and str(r.get("rno")) == str(rno)):
                    continue
                try:
                    uv = json.loads(r.get("uv_json") or "{}")
                except Exception:
                ...
                if not rr.get("flag"):
                    continue
                if (rr.get("box2") or {}).get("picks") or (rr.get("box4") or {}).get("picks"):
                    continue          # 記録済み
                try:
                    rc = find_race(fetch_openapi(hd), int(jcd), int(rno))
                except Exception:
                    rc = None
                ex, tri = _payouts(rc) if rc else (None, None)
                if rc is None or tri is None:
                    remaining += 1; continue   # データ未取得 → 次の周で再挑戦
                pk = _rare_upset_pick(rc, thr=20.0)
                for _bk in ("box2", "box4"):
                    bx = dict(pk.get(_bk) or {})
                    if not bx.get("picks"):
                        continue
                    src = ex if bx.get("type") == "2連単" else tri
                    bh = 0; bret = 0.0
                    if src is not None:
                        for p in bx["picks"]:
                            if p.get("combo") == src[0]:
                                bh = 1; bret += (p.get("stake", 0) or 0) * (float(src[1]) / 100.0)
                    bx["hit"] = bh; bx["ret"] = int(bret)
                    rr[_bk] = bx
                if not rr.get("ab"):
                    rr["ab"] = pk.get("ab") or _box_ab(rc)
                rr["manual_box"] = "2026-09-24 藤田申告: 実際に購入(v126導入前の判定)"
                uv["rare20"] = rr
                r["uv_json"] = json.dumps(uv, ensure_ascii=False)
                changed += 1
                print("[manual-box] %s %s %sR box2=%s ret=%s box4=%s ret=%s" % (
                    hd, jcd, rno, (rr.get("box2") or {}).get("boats"), (rr.get("box2") or {}).get("ret"),
                    (rr.get("box4") or {}).get("boats"), (rr.get("box4") or {}).get("ret")), flush=True)
        if changed:
            _tri_save_ledger(rows)
        if remaining == 0:
            _manual_box_done = True


def _tri_scheduler_loop():
    while True:
        try:
            _tri_manual_box_fill()   # v137
        except Exception as e:
            print("[manual-box]", e, flush=True)
        try:
            if 8 <= _tri_now().hour <= 23:
                tri_judge_cycle()
        except Exception as e:
        ...
        time.sleep(60)   # v129: 150秒→60秒（結果の反映を早くする。判定は1レース1回なので負荷はほぼ同じ）


# ================= v125: オッズ時系列の記録（資金の流れモデル用）=================
# 2026-09-23 藤田指示。背景（claude/boat-race-fresh-look-0923）:
#   公開データ(出走表・展示・気象)だけのモデルは最終オッズに勝てない(混ぜてもモデルの重み0.003)。
#   残る方向は「締切前にどこへお金が入るか」。そのためにオッズの時系列を全レースで貯める。
# 何をするか（記録専用・買い判定には一切使わない・既存の判定ロジック/台帳には触らない）:
#   締切 20・15・10・7・5・3・2・1分前 と 締切後(確定) の9回、
#     単勝・複勝・2連単・3連単 ＋ 2連複/3連複(並び未確認のため生の数値列) を取得。
#   締切10分前・2分前・締切後は公式「直前情報」ページも取得（部品交換・プロペラ・チルト・体重・
#     展示・スタート展示・気象）。並びが未確認の項目は、行ごとのテキストのまま保存して後で解析する。
#   1行1スナップショットの JSONL（oddsts_YYYYMMDD.jsonl）。
# 量: 1日約150レース×9回×4〜5ページ ≒ 6,000リクエスト/日。1日約6〜8MB(gzip後2MB前後)。
import concurrent.futures as _cf

_SNAP_SLOTS = [20, 15, 10, 7, 5, 3, 2, 1]   # 締切何分前に取るか（降順）
_SNAP_BI_SLOTS = {"10", "2", "final"}     # 直前情報ページも取る枠
_SNAP_FINAL = (-8.0, -1.0)              # 締切後この範囲(分)で確定オッズを取りに行く
_SNAP_PATH = os.path.join(_TRI_DIR, "oddsts_{hd}.jsonl")
_snap_done = set()                      # (hd, jcd, rno, slot)
_snap_loaded = set()                    # 既存ファイルを読み込み済みの日
_snap_lock = threading.Lock()


def fetch_winplace_odds(jcd, rno, hd):
    """boatrace.jp oddstf から 単勝(6)・複勝(6, 下限-上限) を取得。失敗時 None。
    ページ内の td.oddsPoint は 単勝6つ → 複勝6つ の順(艇番昇順)。欠場は None。"""
    url = f"https://boatrace.jp/owpc/pc/race/oddstf?rno={rno}&jcd={jcd:02d}&hd={hd}"
    try:
        r = requests.get(url, headers=UA, timeout=10); r.raise_for_status()
    except Exception:
        return None
    soup = BeautifulSoup(r.text, "lxml")
    cells = [c.get_text(strip=True).replace(",", "") for c in soup.select("td.oddsPoint")]
    if len(cells) < 12:
        return None
    win, place = {}, {}
    for i in range(6):
        try:
            win[str(i + 1)] = float(cells[i])
        except ValueError:
            win[str(i + 1)] = None
        t = cells[6 + i]
        m = re.match(r"^\s*([\d.]+)\s*-\s*([\d.]+)\s*$", t)
        if m:
            place[str(i + 1)] = [float(m.group(1)), float(m.group(2))]
        else:
            try:
                v = float(t); place[str(i + 1)] = [v, v]
            except ValueError:
                place[str(i + 1)] = None
    return {"win": win, "place": place}


def _fetch_points(url, timeout=10):
    """td.oddsPoint の数値を並び順のまま返す（並びの意味は後で解析）。失敗時 None。"""
    try:
...
def fetch_exacta_quinella_raw(jcd, rno, hd):
    """odds2tf: 先頭30=2連単(既存の並び)、残り=2連複(並び未確認・生のまま)。"""
    url = f"https://boatrace.jp/owpc/pc/race/odds2tf?rno={rno}&jcd={jcd:02d}&hd={hd}"
    vals, _ = _fetch_points(url)
    ...
    for row_i in range(5):
        for first in range(1, 7):
            v = vals[idx]; idx += 1
            ex[f"{first}-{seconds_by_first[first][row_i]}"] = v if isinstance(v, float) else None
    return ex, vals[30:]


def fetch_trio_raw(jcd, rno, hd):
    """odds3f(3連複20通り)。並び未確認のため生の数値列で返す。"""
    url = f"https://boatrace.jp/owpc/pc/race/odds3f?rno={rno}&jcd={jcd:02d}&hd={hd}"
    vals, _ = _fetch_points(url)
    return vals


def fetch_beforeinfo_text(jcd, rno, hd):
    """直前情報ページを行ごとのテキストで保存（部品交換・プロペラ・チルト・体重・展示・気象）。"""
    url = "https://boatrace.jp/owpc/pc/race/beforeinfo?rno=%d&jcd=%02d&hd=%s" % (int(rno), int(jcd), hd)
    try:
        r = requests.get(url, headers=UA, timeout=10); r.raise_for_status()
    except Exception:
    ...
    for tb in soup.find_all("tbody"):
        ...
        if "is-boatColor" not in " ".join(tds[0].get("class") or []):
            continue
        rows = []
        for tr2 in tb.find_all("tr"):
            rows.append([td.get_text(" ", strip=True) for td in tr2.find_all(["td", "th"])])
        boats[t0] = rows
    weather = [el.get_text(" ", strip=True) for el in soup.select('[class*="weather1_body"]')]
    st = [el.get_text(" ", strip=True) for el in soup.select('[class*="table1_boatImage1"]')]
    if not boats and not weather:
        return None
    return {"boats": boats, "weather": weather, "st": st}


def _snap_inv(d):
    """Σ(1/odds)。控除25%なら約1.33。パースずれの検知用に一緒に保存する。"""
    s = 0.0
...
def _snap_load_day(hd):
    """再起動しても同じスナップショットを二重に取らないよう、既存ファイルから取得済みを復元。"""
    if hd in _snap_loaded:
        return
    _snap_loaded.add(hd)
    path = _SNAP_PATH.format(hd=hd)
    ...
    try:
        with open(path, encoding="utf-8") as f:
            for line in f:
                try:
                    j = json.loads(line)
                    _snap_done.add((hd, int(j["jcd"]), int(j["rno"]), str(j["slot"])))
                except Exception:
                    continue
    except Exception as e:
        print("[oddsts] load", e, flush=True)


def _snap_take(hd, jcd, rno, closed, slot, mins):
    wp = fetch_winplace_odds(jcd, rno, hd) or {}
    ex, qu_raw = fetch_exacta_quinella_raw(jcd, rno, hd)
    tri = fetch_trifecta_odds(jcd, rno, hd) or {}
    trio_raw = fetch_trio_raw(jcd, rno, hd)
    bi = fetch_beforeinfo_text(jcd, rno, hd) if str(slot) in _SNAP_BI_SLOTS else None
    if not (wp or ex or tri or bi):
        return False
    rec = {"ts": _tri_now().strftime("%Y-%m-%d %H:%M:%S"), "date": hd, "jcd": jcd, "rno": rno,
           "closed_at": closed.strftime("%H:%M"), "slot": str(slot), "mins": round(mins, 2),
           "win": wp.get("win"), "place": wp.get("place"), "ex": ex or None, "tri": tri or None,
           "qu_raw": qu_raw or None, "trio_raw": trio_raw or None, "bi": bi,
           "chk": {"win": _snap_inv(wp.get("win")), "ex": _snap_inv(ex), "tri": _snap_inv(tri)}}
    line = json.dumps(rec, ensure_ascii=False, separators=(",", ":"))
    with _snap_lock:
        with open(_SNAP_PATH.format(hd=hd), "a", encoding="utf-8") as f:
            f.write(line + "\n")
    return True


def odds_snapshot_cycle():
    now = _tri_now()
    hd = now.strftime("%Y%m%d")
    _snap_load_day(hd)
    try:
    ...
    for jcd in range(1, 25):
        for rno in range(1, 13):
            ...
            mins = (closed - now).total_seconds() / 60.0
            slot = None
            for i, s in enumerate(_SNAP_SLOTS):
                # s分前ちょうど〜次の枠の手前までに1回だけ取る(取り逃したらその枠は空のまま)
                nxt = _SNAP_SLOTS[i + 1] if i + 1 < len(_SNAP_SLOTS) else 0
                if nxt < mins <= s:
                    slot = s
                    break
            if slot is None and _SNAP_FINAL[0] <= mins <= _SNAP_FINAL[1]:
                slot = "final"
            if slot is None:
                continue
            key = (hd, jcd, rno, str(slot))
            if key in _snap_done:
                continue
            _snap_done.add(key)
            jobs.append((hd, jcd, rno, closed, slot, mins))
    if not jobs:
        return 0
    ok = 0
    with _cf.ThreadPoolExecutor(max_workers=6) as pool:
        for r in pool.map(lambda a: _snap_safe(*a), jobs):
            ok += 1 if r else 0
    return ok
...
@app.get("/api/odds/ts.jsonl", response_class=PlainTextResponse)
def api_odds_ts(date: str = ""):
    """v125: その日のオッズ時系列(JSONL)。GitHub Actions が毎晩 data/oddsts/ へコミットする。"""
...
@app.get("/api/odds/ts/status")
def api_odds_ts_status(date: str = ""):
    """v125: オッズ時系列の収集状況（直前情報の取得数 bi も表示）。枠(slot)ごとの件数と、Σ(1/odds) の中央値(約1.33なら正常)。"""
    hd = date or _tri_now().strftime("%Y%m%d")
    path = _SNAP_PATH.format(hd=hd)
    if not os.path.exists(path):
        return {"date": hd, "exists": False, "snapshots": 0}
    by, races, chk, nbi = {}, set(), {"win": [], "ex": [], "tri": []}, 0
    last = None
    with open(path, encoding="utf-8") as f:
        for line in f:
            try:
                ...
                continue
            by[j["slot"]] = by.get(j["slot"], 0) + 1
            if j.get("bi"):
                nbi += 1
            races.add((j["jcd"], j["rno"]))
            last = j["ts"]
            for k in chk:
                v = (j.get("chk") or {}).get(k)
                if v:
                    chk[k].append(v)
    med = {k: (sorted(v)[len(v) // 2] if v else None) for k, v in chk.items()}
    return {"date": hd, "exists": True, "races": len(races), "snapshots": sum(by.values()),
            "by_slot": by, "beforeinfo": nbi, "median_sum_inv": med, "last": last, "bytes": os.path.getsize(path)}
...
def _tri_start():
    ...
    threading.Thread(target=_snap_loop, daemon=True).start()   # v125 オッズ時系列＋直前情報
...
def api_tri_summary():
    ...
    def agg_rare_box(rs, field, box, ex_ok_only=True):
        """v126: 波乱BOXの通算。展示3位以下(ex_skip False)の分だけ。"""
        buy = hits = 0; stake = 0.0; ret = 0.0
        for r in rs:
            try:
                u = json.loads(r.get("uv_json") or "{}")
            ...
            if ex_ok_only and rr.get("ex_skip") is not False:
                continue
            if ex_ok_only and not _box_ok(rr):   # v132: A・Bどちらも無しは買っていない
                continue
            bx = rr.get(box) or {}
            if rr.get("flag") and bx.get("picks") and r.get("status") == "settled" and str(bx.get("hit", "")) != "":
                buy += 1
                stake += sum((p.get("stake", 0) or 0) for p in bx["picks"])
                hits += 1 if str(bx.get("hit")) == "1" else 0
                ret += fnum(bx.get("ret"))
        return {"buy": buy, "hits": hits, "stake": int(stake), "ret": int(ret),
                "roi": round(ret / stake * 100, 1) if stake else 0}

    def agg_rare_box_ab(rs, mode):
        """v132: 波乱BOX(2連単＋3連単合計)の A・B 別の通算。"""
        buy = hits = 0; stake = 0.0; ret = 0.0
        for r in rs:
            try:
                u = json.loads(r.get("uv_json") or "{}")
            ...
            v = u.get("rare20") or {}
            ab = v.get("ab")
            if not (v.get("flag") and ab and v.get("ex_skip") is False and r.get("status") == "settled"):
                continue
            if mode in ("orA", "orB"):   # v135: AかB × 1号艇A級/B級
                if not ab.get("c1"):
                    continue
                ok = bool(ab.get("A") or ab.get("B")) and ((ab.get("c1") in ("A1", "A2")) == (mode == "orA"))
            else:
            ...
            if not ok:
                continue
            bxs = [b for b in (v.get("box2") or {}, v.get("box4") or {}) if b.get("picks")]
            if not bxs or any(str(b.get("hit", "")) == "" for b in bxs):
                continue
            buy += 1
            stake += sum((p.get("stake", 0) or 0) for b in bxs for p in b["picks"])
            ret += sum(fnum(b.get("ret")) for b in bxs)
            hits += 1 if any(str(b.get("hit")) == "1" for b in bxs) else 0
        return {"buy": buy, "hits": hits, "stake": int(stake), "ret": int(ret),
                "roi": round(ret / stake * 100, 1) if stake else 0}
    ...
    return {"ok": True, "hd": hd, "dates": dates,
            "today": {"hit": agg_hit(today), "uv40": agg_uv(today, "dec40"), "uv55": agg_uv(today, "dec55"),
                      ...
                      "rare_box2": agg_rare_box(today, "rare", "box2"), "rare_box4": agg_rare_box(today, "rare", "box4"),
                      "rare20_box2": agg_rare_box(today, "rare20", "box2"), "rare20_box4": agg_rare_box(today, "rare20", "box4"),
                      "rare20_box_AorB": agg_rare_box_ab(today, "or"), "rare20_box_AandB": agg_rare_box_ab(today, "and"),
                      "rare20_box_AorB_Acls": agg_rare_box_ab(today, "orA"), "rare20_box_AorB_Bcls": agg_rare_box_ab(today, "orB"),
            ...
            "cumulative": {"hit": agg_hit(rows), "uv40": agg_uv(rows, "dec40"), "uv55": agg_uv(rows, "dec55"),
                           ...
                           "rare_box2": agg_rare_box(rows, "rare", "box2"), "rare_box4": agg_rare_box(rows, "rare", "box4"),
                           "rare20_box2": agg_rare_box(rows, "rare20", "box2"), "rare20_box4": agg_rare_box(rows, "rare20", "box4"),
                           "rare20_box_AorB": agg_rare_box_ab(rows, "or"), "rare20_box_AandB": agg_rare_box_ab(rows, "and"),
                           "rare20_box_AorB_Acls": agg_rare_box_ab(rows, "orA"), "rare20_box_AorB_Bcls": agg_rare_box_ab(rows, "orB"),
