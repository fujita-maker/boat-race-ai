...
def final_page():
    ...
    if not rows:
        rows = _gh_ledger("tri_hit_ledger.csv") or []
    ...
    head = ('<!doctype html><html lang="ja"><head><meta charset="utf-8">'
            ...
            '<h1>&#128337; 締切5分前 最終判定（3連単 的中率重視）</h1>' % css)
    ...
    def card(r):
        ...
        try:
            ...
            uv = {}
        if uv.get("decision") == "買い" and uv.get("key"):
            k = uv["key"]
            up = " / ".join('<span class="combo">%s</span>(%s倍)' % (p.get("combo"), p.get("odds") if p.get("odds") is not None else "—")
                            for p in uv.get("picks", []))
            uvres = ""
            if r.get("status") == "settled" and r.get("win_combo"):
                uvres = ('　<span class="res-hit">的中%s円</span>' % f"{int(fnum(r.get('uv_ret'))):,}"
            ...
            h += ('<div style="background:#fff8e6;border:1px solid #f0e2b8;border-radius:10px;padding:8px 10px;margin-top:8px;font-size:13px">'
                  '<b style="color:#b8860b">&#127775; 過小評価キー買い</b>：<b>%s号艇</b>(モ%s%%/全国2連率%s%%)を軸ヒモ　%s%s</div>') % (
                  k.get("frame"), round(fnum(k.get("motor2"))), round(fnum(k.get("nat2"))), up, uvres)
        # 結果
        if r.get("status") == "settled" and r.get("win_combo"):
            if isbuy:
    ...
    return HTMLResponse(head + bar + kpi + body + note + "</body></html>")
...
def undervalue_pick(race, odds_map):
    ...
    combos = [((P, Q, X), 200, "3着"), ((Q, P, X), 200, "3着"), ((P, R, X), 200, "3着"),
              ((P, X, Q), 150, "2着"), ((Q, X, P), 150, "2着"), ((X, P, Q), 100, "1着(上振れ)")]
    tri = trifecta_probs_pl(power)
    picks = []
    exp_ret = 0.0; total = 0
