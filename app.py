...
def final_page():
    ...
    uv_shadow = ('<div class="fcard" style="border-color:#dbe2ea">'
                 ...
                 '&#128203; 記録のみのバケット（買わない・検証用）　%d件</div>' % ((35 if _MIDEV_DISPLAY else 31 + len(_MIDEV_SHADOWS)) + (0 if _F_LIVE else 1) + (0 if _B_LIVE else 1) + 1 + (0 if _C_LIVE else 1) + (0 if _G_LIVE else 1))
    ...
    def card(r):
        ...
        _plans = [lab for ok, lab in ((_planA, "A案"), (_planB, "B案"), (bool(_cpk), "C案"), (bool(_dpk), "D案"),
                                      (bool(_fpk), "F案"), (bool(_gpk), "G案")) if ok]
        if _rarebuy:
        ...
        if isbuy:
            ...
            h += '<div class="tsub" style="margin:4px 0">実弾の枠（A案・D案・E案・F案・G案・波乱）はどれも条件に当てはまらないため買いません。%s</div>' % (
        ...
        if uv:
            ...
            h += ('<div style="background:#fff8e6;border:1px solid #f0e2b8;border-radius:10px;padding:8px 10px;margin-top:8px;font-size:13px">'
                  '%s%s%s%s%s%s%s%s</div>') % (badge, inner, _lane_line(uv), _lane_real_line(uv),
                                          ...
                                          _midev_line(uv) + _fplan_line(uv) + _g_line(uv))
        ...
        try:
            ...
            if _gb:
                ...
                for ca, r, gv in _gb:
                    ...
                    g8 += ('<tr><td><b style="color:#1c7a38">&#128994; 買い</b></td><td>%s</td><td><b>%s%sR</b></td>'
            ...
            g8 = ""
        return fh + gh + g8 + h      # v96: E案の候補テーブルを上に出す／v103: その下にF案／v108: その下にG案
    # ---- 通算（全期間）の回収率と95%信頼区間 ----
    ...
    return HTMLResponse(head + bar + uv_live + cand_block + midev_today + rare10_block
                        ...
                        + kpi_ref + note + _CD_SCRIPT + "</body></html>")
