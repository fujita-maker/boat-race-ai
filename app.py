            ...
            try:
                if plan == "A":
                    ...
                    if v0.get("flag") and v0.get("ex_skip") is False and _box_ok(v0):   # v132
                        ...
                        if bx.get("picks"):
                            items = [("%s BOX %s<span class=\"tsub\">(%d点)</span>" % (bx.get("type"), "".join(str(x) for x in bx.get("boats", [])), len(bx["picks"])),
    ...
    uv_live = ('<div class="fcard" style="border-color:#1c7a38;background:#f4fbf6">'
               ...
               + ((_uv_row("波乱BOX①：2連単BOX（1号艇以外の総合力上位3艇・6点×1,000円／波乱の目≤20%かつ展示で1号艇3位以下）",
                   ...
                   + _uv_row("波乱BOX（2連単＋3連単）のうち「AとB両方」に当てはまるレース",
                             _rare_box_ab_stats("and"), live=False, color="#b23b3b", tag="内訳")) if _RARE_BOX_LIVE else "")
    ...
    def card(r):
        ...
        if isbuy:
            ...
            for _bk, _bx in _rbox:
                ...
                h += ('<tr><td>波乱BOX</td><td class="combo">%s (%d点)</td><td>—</td><td>%s円</td><td>%s</td></tr>'
...
def _ab_box_html(rr):
    ...
    return ('<div style="display:inline-block;text-align:center;border:2px solid %s;background:%s;border-radius:8px;'
