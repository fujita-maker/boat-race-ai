...
def _form_line(uv):
    ...
    if gv.get("flag"):
        return ('<div class="tsub" style="margin-top:2px"><b style="color:#1c7a38">E案 買い</b>'
                '<span style="color:#8a97a5">（1着1号艇・2着%s・3着全の8点×%s円／8点平均EV %.2f）</span>'
                '　%s%s</div>') % ("・".join(str(x) for x in gv["second"]), f"{_FORM_UNIT:,}",
                                  _fnum(gv.get("ev1")), body, tail)
    return ('<div class="tsub" style="margin-top:2px">E案 見送り'
            '<span style="color:#8a97a5">（%s）</span>%s</div>') % (gv.get("reason", ""), tail)
...
def final_page():
    ...
    uv_shadow = ('<div class="fcard" style="border-color:#dbe2ea">'
                 ...
                 + _uv_row("仮説H2：キー艇3〜6号艇 → 1着1号艇・2着キーの内隣・3着全（4点×%d円）" % _H2_UNIT,
                           _h2_stats())
                 + _uv_row("E案：EVが1.0未満で見送ったレース（もし買っていたら・比較用）", _form_stats(False))
                 + _uv_row("E案：EVを見ずに形だけで全部買った場合（実測91%が上限の形）", _form_stats(None))
