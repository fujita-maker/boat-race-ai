...
_MIDEV_N = 5                        # 最大点数
# v89(2026-09-20 藤田指示「実践でするから書き換えて」): 200 → 1000。
#   実際に買っている額は1点1,000円であり、コードの200円は実態と5倍ずれていた。
...
#     gap>=0.5 の実測は1日9〜25レース(平均12.8)なので、1,000円では1日9,000〜25,000円になる。
...
#   そこで「その日の早い者勝ちで上限レースまでを実弾、それ以降は記録のみ」にする設計だった。
#   ⚠ v71で _MIDEV_DISPLAY(影バケットを実弾化)を入れて以降、この上限は C案の買い判定には効いていない。
#     現在は条件を満たした全レースを買う。上限を復活させるなら _midev_today_block_disp と
#     _cpk(2865行付近)で mv.get("live") を見るようにする必要がある。
