...
def final_page():
    ...
    uv_kpi = (_uv_row("≥40混戦（B案・BT94.8%）", uv40)
              + _uv_row("≥40混戦＋1号艇展示良好（B案改良・BT100.4%）", uv40_ex)
              + _uv_row("≥40混戦＋穏やか水面(波&lt;4cm)（B案改良2・BT101.0%）", uv40_calm)
              + _uv_row("≥40混戦＋期待値ev&ge;1.2（v59影・ライブ80.5%）", uv40_ev12)
              + _uv_row("≥40混戦＋期待値ev&ge;1.3（v59影・ライブ85.7%）", uv40_ev13)
    ...
    def _uv_candidates():
        ...
        if n == 0:
            ...
            h += ('<table style="margin-top:8px"><tr><th>締切</th><th>場R</th><th>対象</th>'
                  '<th>キー</th><th>モ/全国2連</th><th>pgap</th>'
                  '<th style="text-align:left">買い目（朝の時点・v62=4点600円）</th><th>状態</th></tr>')
            for c in cands:
                ...
                h += ('<tr><td>%s</td><td class="ct">%s%sR</td><td><b>%s</b></td>'
                      '<td>%s号(1着%s%%)</td><td>%s/%s%%</td><td>%.2f%s</td>'
                    ...
                    round(fnum(c["nat2"])), c["pgap"], ("*" if bd else ""), buy_s, st)
            h += '</table>'
