...
def final_page():
    ...
    uv_live = ('<div class="fcard" style="border-color:#1c7a38;background:#f4fbf6">'
               ...
               + '<div class="tsub" style="margin-top:8px">'
                 + ('A案・B案（過小評価キー）・C案（中オッズEV）・D案（低中オッズ較正EV）を並行してライブ検証中。'
                    ...
                    '判定時オッズで的中20〜30本たまったら見直します。A案・C案・D案・E案も並行してライブ検証中。')
                 + 
    ...
    uv_shadow = ('<div class="fcard" style="border-color:#dbe2ea">'
                 ...
                 '&#128203; 記録のみのバケット（買わない・検証用）　%d件</div>' % ((35 if _MIDEV_DISPLAY else 31 + len(_MIDEV_SHADOWS)) + (0 if _F_LIVE else 1) + (0 if _B_LIVE else 1))
    ...
    def card(r):
        ...
        if isbuy:
            ...
            h += '<div class="tsub" style="margin:4px 0">実弾の枠（A案・C案・D案・F案）はどれも条件に当てはまらないため買いません。%s</div>' % (
    ...
    def _uv_candidates():
        ...
        try:
          if cands:
              ...
              _nxt = ('<div style="margin-top:8px;padding:10px 12px;background:#f4fbf6;'
                      ...
                      '<span style="font-size:13px;color:#5c6b7a">次の過小評価キー候補（A案＝実弾／B案＝記録のみ）は</span>　'
        ...
        if ebuys:
            ...
            for ca, r, gv in ebuys:
                ...
                fh += ('<tr><td><b style="color:#1c7a38">&#128994; 買い</b></td><td>%s</td><td><b>%s%sR</b></td>'
        ...
        try:
            ...
            if fbuys:
                ...
                for ca, r, fv in fbuys:
                    ...
                    gh += ('<tr><td><b style="color:%s">&#128994; %s</b></td><td>%s</td><td><b>%s%sR</b></td><td style="text-align:left">%s</td><td>%s円</td><td>%s</td></tr>'
            ...
            if fupcoming:
                ...
                gh += ('<div class="tsub" style="margin-top:8px">次に判定されるレース（締切の5〜10分前に判定）　%s%s</div>'
                       % ("　".join('<b>%s</b> %s%dR' % (c.strftime("%H:%M"), ven(j), rn) for c, j, rn in nx),
                          ("　ほか%d件" % (len(fupcoming) - len(nx))) if len(fupcoming) > len(nx) else ""))
            gh += '</div>'
        except Exception as _e:
            print("[F案ブロック]", _e, flush=True)
            gh = ""
        return fh + gh + h      # v96: E案の候補テーブルを上に出す／v103: その下にF案
    # ---- 通算（全期間）の回収率と95%信頼区間 ----
