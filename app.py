...
def final_page():
    ...
    def _rare_block(field="rare", thr=10.0):
        ...
        if n == 0:
            ...
            for c in cands:
                ...
                if exr is None:
                    ex_s = '<span class="tsub">未公開</span>'
                    jd = '<b style="color:#8a6d1f">&#9203; 展示待ち</b><br><span class="tsub" style="font-size:11px">締切10分前ごろ判明</span>'
                    row_style = ''
                elif exr["rank"] <= _RARE_EX_SKIP_RANK:
                    ex_s = '<b>%s位</b>（%.2f）' % (exr["rank"], exr["ex1"])
        ...
        if field == "rare":
            h += ('<div class="tsub" style="margin-top:6px"><b>見方：</b>朝から該当レースと買い目を出し、'
                  '<b>展示タイム（締切の約10〜12分前に公開・この画面は2分ごとに自動更新）で1号艇が2位以内なら「🔴見送り」</b>、'
    ...
    return HTMLResponse(head + bar + kpi + uv_kpi + cum_block + midev_block + midev_today + rare10_block + cand_block + rare_block + body + note + "</body></html>")
