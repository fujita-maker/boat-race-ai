...
def final_page():
    ...
    uv_live = ('<div class="fcard" style="border-color:#1c7a38;background:#f4fbf6">'
               ...
               + (_uv_row("G案：締切前に売れた中オッズの1点（判定時10〜50倍→締切%g〜%g分前に%d%%以上下落した組のうち最大の1点×%d円）%s" % (_G_HI, _G_LO, int(100 * _G_DROP), _G_UNIT, "" if _G_LIVE else "・記録のみ"), _gplan_stats(), live=_G_LIVE, color=_G_COLOR, plan="G") if _G_LIVE else "")
        ...
        try:
            ...
            if _gb:
                g8 += ('<table style="margin-top:8px"><tr><th></th><th>締切</th><th>場R</th><th>買い目</th>'
                       '<th>判定時→決定時</th><th>展示タイム（1〜6号艇）・進入</th><th>金額</th><th>結果</th></tr>')
                for ca, r, gv in _gb:
                    pk = (gv.get("picks") or [{}])[0]
                    if r.get("status") == "settled" and r.get("win_combo"):
                    ...
                    g8 += ('<tr class="%s" data-k="%s"><td><b style="color:#1c7a38">&#128994; 買い</b></td><td>%s</td><td><b>%s%sR</b></td>'
                           '<td><span class="combo" style="color:%s">%s</span></td><td>%s倍→%s倍（%d%%下落）</td><td>%s</td><td>%s円</td><td>%s</td></tr>'
                           % ("buyrow" if _G_LIVE else "", "G案|%s%sR|%s|締切%s" % (ven(r.get("jcd")), r.get("rno"), pk.get("combo"), ca),
                              ...
                              round(-100 * fnum(pk.get("drop"))), _exlane(r.get("jcd"), r.get("rno"), r),
