...
def _maezuke_master():
    """選手別の内寄せ率(前づけ率)マスタ。data/maezuke_master.json をGitHubから読む(1時間キャッシュ)。
    取得できなければ None（進入リスク表示だけが消える。買い判定には影響しない）。"""
    now = time.time()
    if _MZ_CACHE["d"] is not None and now - _MZ_CACHE["t"] < 3600:
        return _MZ_CACHE["d"]
...
def final_page():
            ...
            for c in cands:
                ...
                if c.get("lane"):
                    ...
                    sn = ('<span style="color:%s;font-weight:700" title="%s">%s</span>'
                        ...
                        lcol, " / ".join(sr.get("names") or []) or "前づけ常習者なし",
    ...
    def _rare_block():
        try:
            ...
            data = None
        if not data:
            return _pending_block("#b8860b",
                                  "&#127775; 希少・波乱の目（1号艇モーター激弱 ≤10%＝約1/140の希少）")
        settled = {}
        for r in today:
            try:
                u = json.loads(r.get("uv_json") or "{}")
                ...
                u = {}
            if (u.get("rare") or {}).get("flag"):
                settled[(str(r.get("jcd")), str(r.get("rno")))] = (r, u.get("rare"))
        cands = []
        for jcd in range(1, 25):
            for rno in range(1, 13):
                rc = find_race(data, jcd, rno)
        ...
        n = len(cands)
        h = ('<div class="fcard" style="border-color:#b8860b">'
             '<div class="tsub" style="font-weight:700;color:#b8860b">'
             '&#127775; 希少・波乱の目（1号艇モーター激弱 ≤10%%＝約1/140の希少）　%d件</div>' % n)
        if n == 0:
            h += '<div class="tsub" style="margin-top:6px">本日は該当なし（1号艇のモーターが極端に弱いレースが出れば表示）。</div>'
        else:
            h += '<table style="margin-top:8px"><tr><th>締切</th><th>場R</th><th>1号艇モ</th><th>推奨買い目(最強機力頭6点=600円)</th><th>結果</th></tr>'
            for c in cands:
                ...
                if c["led"]:
                    ...
                    res = "締切後"
                h += ('<tr><td>%s</td><td class="ct">%s%sR</td><td>%.1f%%</td>'
        ...
        h += ('<div class="tsub" style="margin-top:6px">※あなたの着眼（人気の1号艇がモーター激弱→波乱）を前向き記録する検証枠。'
              '<b>過去182日は該当180レース・的中わずか5本(2.8%)で、回収率は5月の1発次第＝サンプル僅少の完全ノイズ＝買い非推奨</b>'
              '（激弱でも1号艇は52%勝つ）。希少なので実弾は最小(600円)、あくまで前向きにデータを貯めるための枠。'
              '通算は /api/tri/summary の rare_upset で自動集計。</div></div>')
        return h
    rare_block = _rare_block()
    ...
    return HTMLResponse(head + bar + kpi + uv_kpi + cum_block + cand_block + rare_block + body + note + "</body></html>")
    ...
    with _tri_lock:
            ...
            for rno in range(1, 13):
                ...
                try:
                    ...
                    uv_json = json.dumps({"decision": uv["decision"],
                                          ...
                                          "reason": uv["reason"], "rare": _rare_upset_pick(rc)}, ensure_ascii=False)
                except Exception:
                    uv_json = ""
                rows.append({"ts": now.strftime("%Y-%m-%d %H:%M:%S"), "date": hd, "jcd": jcd, "rno": rno,
        ...
        for r in rows:
            ...
            if rr.get("flag") and rr.get("picks"):
                ...
                rr["hit"] = rhit; rr["ret"] = int(rret); uv["rare"] = rr
