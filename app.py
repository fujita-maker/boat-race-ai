...
# 何をするか:
#   締切 15・10・5・2分前 と 締切後(確定オッズ) の5回、単勝・複勝・2連単・3連単を取得して
#   1行1スナップショットの JSONL で保存する。買い判定には一切使わない(記録専用)。
#   既存の判定ロジック・台帳には触らない。
# 量: 1日約150レース×5回×4ページ ≒ 3,000リクエスト/日。ファイルは1日約2〜3MB(gzip後1MB未満)。
import concurrent.futures as _cf

_SNAP_SLOTS = [15, 10, 5, 2]            # 締切何分前に取るか
...
def _snap_take(hd, jcd, rno, closed, slot, mins):
    ...
    ex = fetch_exacta_odds(jcd, rno, hd) or {}
    tri = fetch_trifecta_odds(jcd, rno, hd) or {}
    if not (wp or ex or tri):
        return False
...
def odds_snapshot_cycle():
    ...
    for jcd in range(1, 25):
        for rno in range(1, 13):
            ...
            mins = (closed - now).total_seconds() / 60.0
            slot = None
            for s in _SNAP_SLOTS:
                # s分前ちょうど〜(s-2)分前の間に1回だけ取る(取り逃したらその枠は空のまま)
                if s - 2.0 < mins <= s:
                    slot = s
                    break
    ...
    with _cf.ThreadPoolExecutor(max_workers=4) as ex:
        for r in ex.map(lambda a: _snap_safe(*a), jobs):
            ok += 1 if r else 0
    return ok
...
def api_odds_ts_status(date: str = ""):
    """v125: オッズ時系列の収集状況。枠(slot)ごとの件数と、Σ(1/odds) の中央値(約1.33なら正常)。"""
    hd = date or _tri_now().strftime("%Y%m%d")
    path = _SNAP_PATH.format(hd=hd)
    if not os.path.exists(path):
        return {"date": hd, "exists": False, "snapshots": 0}
    by, races, chk = {}, set(), {"win": [], "ex": [], "tri": []}
    last = None
    with open(path, encoding="utf-8") as f:
        for line in f:
            try:
    ...
    return {"date": hd, "exists": True, "races": len(races), "snapshots": sum(by.values()),
            "by_slot": by, "median_sum_inv": med, "last": last, "bytes": os.path.getsize(path)}
...
def _tri_start():
    ...
    threading.Thread(target=_snap_loop, daemon=True).start()   # v125 オッズ時系列
