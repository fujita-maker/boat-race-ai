...
def formation_pick(boats, odds_map):
    ...
    if fast < 4:
        return dict(base, flag=False, reason="展示最速が%d号艇(外枠4〜6のときだけ)" % fast)
    ...
    base = dict(base, ev1=round(ev1, 2), picks=picks, total=_FORM_UNIT * len(picks))
    if ev1 < _FORM_EV:
        return dict(base, flag=False,
                    reason="8点の平均EV %.2f が %.1f 未満＝この値段では割安でない" % (ev1, _FORM_EV))
    return dict(base, flag=True,
                reason=("1着1号艇・2着%s(展示最速%d号艇とその内隣)・3着全の8点。"
                        "8点平均EV %.2f＝市場が外枠2着を安く売っている。"
                        % ("/".join(str(x) for x in second), fast, ev1)))
...
def final_page():
    ...
    uv_live = ('<div class="fcard" style="border-color:#1c7a38;background:#f4fbf6">'
               ...
               + _uv_row("E案：フォーメーション 1着1号艇・2着=展示最速とその内隣・3着全"
                         "（8点×%d円・8点平均EV&ge;%.1fのときだけ）" % (_FORM_UNIT, _FORM_EV),
               ...
               + '<div class="tsub" style="margin-top:8px">'
                 ...
                 '<br>&#9888;&#65039; <b>カードで「&#128994; 買い」になる枠は上の4つだけではありません。</b>'
    ...
    def _uv_candidates():
            ...
            for rno in range(1, 13):
                ...
                if _gv:
                    if _gv.get("second") and _gv.get("fast") and _gv.get("spread") is not None \
                            and _gv["fast"] >= 4 and _gv["spread"] >= _FORM_SPREAD \
        ...
        fh = ('<div class="fcard" style="border-color:%s;border-width:2px;background:%s">'
              ...
              '&#128995; 本日の E案 候補（1着1号艇・2着=展示最速とその内隣・3着全／未締切）　%d件'
              '<span style="font-weight:400;color:#5c6b7a">　＋展示タイム待ち %d件</span></div>'
              % (_FORM_COLOR, _FORM_BG, _FORM_COLOR_D, len(fcands), fwait))
        fh += ('<div class="tsub" style="margin-top:6px;font-size:12px">'
               'E案は<b>展示タイムが出てから</b>でないと買い目が決まりません（展示は締切の30〜40分前）。'
               'ここに出るのは形の条件（1号艇が総合力1位・展示最速が外枠・展示差&ge;%.2f）を満たしたレースで、'
               '<b>最終的に買うかどうかは締切5分前のオッズで決まります</b>'
               '（8点の平均EVが%.1f以上なら買い）。朝の時点では展示が無いので候補は出ません。</div>'
               % (_FORM_SPREAD, _FORM_EV))
        if fcands:
            fh += ('<table style="margin-top:8px"><tr><th>締切</th><th>場R</th><th>展示最速</th>'
            ...
            for c in fcands:
                ...
                fh += ('<tr><td>%s</td><td><b>%s%dR</b></td><td>%d号艇</td><td>%.2f</td><td>%s%%</td>'
                    ...
                    c.get("model_p1"), " ".join(cb), "判定済" if c["judged"] else "EV判定待ち")
            fh += '</table>'
        else:
        ...
        return fh + h      # v96: E案の候補テーブルを上に出す
    # ---- 通算（全期間）の回収率と95%信頼区間 ----
