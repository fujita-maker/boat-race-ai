...
def api_tri_summary():
    ...
    def agg_uv(rs):
        ...
        for r in rs:
            try:
                ...
                u = {}
            if u.get("decision") == "買い" and r.get("status") == "settled":
                bset.append((r, u))
            elif u.get("decision") == "見送り":
                skip += 1
    ...
    return {"ok": True, "hd": hd, "dates": dates,
            "today": {"hit": agg_hit(today), "uv": agg_uv(today)},
            "cumulative": {"hit": agg_hit(rows), "uv": agg_uv(rows)}}
...
def undervalue_pick(race, odds_map):
    ...
    # 妙味の買い条件: 明確な過小評価キー(gap>=3 かつ モーター>=55) かつ 高オッズ(最大オッズ>=15)
    # ※モーター閾値は13か月検証で最適化: >=40は回収率71%だが >=55で90%(過学習チェック済)。精度優先で55に厳格化。
    strong = (gap[X] >= 3 and mot[X] >= 55)
    ...
    if not odds_map:
        ...
        if not strong:
            reason += " ただし過小評価の度合いが基準未満(gap<3 か モーター<55)＝妙味不足で見送り。"
        else:
