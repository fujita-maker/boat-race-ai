...
def final_page():
    ...
    def _uv_stats(field):
        buys = [(r, u) for r, u in uv_today if _uv_dec_or_decision(u, field) == "買い"]
    ...
    uv_shadow = ('<div class="fcard" style="border-color:#dbe2ea">'
                 ...
                 '&#128203; 記録のみのバケット（買わない・検証用）　%d件</div>' % (15 if _MIDEV_DISPLAY else 12 + len(_MIDEV_SHADOWS))
    ...
    def _uv_pairs_all(field):
        ...
        for r in rows:
            ...
            if not u:
                continue
            if _uv_dec_or_decision(u, field) != "買い":
                continue
            if r.get("status") != "settled":
                continue
