...
@app.get("/")
def index():
    # トップは高オッズEVモデル。従来の妙味スコア画面は /myoumi に退避。
    return HTMLResponse(_HIGHODDS_HTML)
...
def dashboard():
    ...
    cardB, cumB = _card("的中(見送り)", "#1c7a38", "GBM上位3点・自信度上位20%", hit)
    ...
    html = ('<!doctype html><html lang="ja"><head><meta charset="utf-8">'
            ...
            '<a class="g" href="/">高オッズ判定ページへ</a></div>'
...
def today_page():
    ...
    head = ('<!doctype html><html lang="ja"><head><meta charset="utf-8">'
            ...
            '.pbuy{background:#1c7a38}.pskip{background:#8a97a5}'
            '.combo{font-weight:700}</style></head><body>'
            '<h1>&#127919; 今日の買い目予想</h1>') % _DASH_CSS
    bar = ('<div class="bar"><a href="/today">&#8635; 最新に更新</a>'
           '<a class="g" href="/dashboard">成績ダッシュボード</a>'
           '<a class="g" href="/">高オッズ判定</a></div>')
    if not d or not d.get("races"):
        body = ('<div class="sub">まだ今日の予想がありません。予想ワークフロー(daily-predict)が'
                '1回動くと表示されます（1日数回・締切に近いほどオッズは最終値に近づきます）。更新: %s</div>' % now)
        return HTMLResponse(head + bar + body + "</body></html>")
    gen = d.get("generated_at", "?"); hd = d.get("hd", "")
    races = d["races"]
    # どちらかが「買い」のレースだけ抽出し、締切順に
    buys = [r for r in races if (r.get("hit") and r["hit"].get("buy")) or
            (r.get("ho") and r["ho"].get("decision") == "買い")]
    ...
    def render_rows(rows):
        h = ('<table><tr><th>締切</th><th>場</th><th>R</th>'
             '<th>的中モデル(上位3点)</th><th>高オッズEV</th></tr>')
        for r in rows:
            ca = str(r.get("closed_at") or "")
            ct = ca[11:16] if len(ca) >= 16 else ca
            hit = r.get("hit"); ho = r.get("ho")
            if hit and hit.get("combos"):
                pill = ('<span class="pill pbuy">買い</span>' if hit.get("buy")
                        else '<span class="pill pskip">様子見</span>')
                hitcell = '%s <span class="combo">%s</span> <span class="tsub">自信%.0f%%</span>' % (
                    pill, " / ".join(hit["combos"]), 100 * hit.get("conf", 0))
            else:
                hitcell = '<span class="tsub">—</span>'
            if ho and ho.get("decision") == "買い" and ho.get("picks"):
                ps = " / ".join("%s(<span class=tsub>%sx EV%s</span>)" % (p["combo"], p["odds"], p["EV"])
                                for p in ho["picks"])
                hocell = '<span class="pill pbuy">買い</span> <span class="combo">%s</span>' % ps
            else:
                hocell = '<span class="tsub">見送り</span>'
            h += ('<tr><td>%s</td><td>%s</td><td>%s</td><td style="text-align:left">%s</td>'
                  '<td style="text-align:left">%s</td></tr>') % (ct, ven(r["jcd"]), r["rno"], hitcell, hocell)
        return h + "</table>"

    hit_n = sum(1 for r in races if r.get("hit") and r["hit"].get("buy"))
    ho_n = sum(1 for r in races if r.get("ho") and r["ho"].get("decision") == "買い")
    sub = ('<div class="sub">%s の未開催レース。的中モデルの「買い」%d件・高オッズの「買い」%d件。'
           '予想生成: %s（本ページは10分ごと自動更新）。'
           '<br><span class="tsub">※オッズは生成時点の値。締切直前が最終値なので、実際に賭ける直前に高オッズ判定で再確認を。</span>'
           '更新: %s</div>') % (hd, hit_n, ho_n, gen, now)
    if not buys:
        table = ('<div class="rowsec"><div class="tsub">今のところ両モデルとも「買い」推奨はありません'
                 '（見送り）。時間をおくと更新されます。</div></div>')
    else:
        table = '<div class="rowsec">' + render_rows(buys) + '</div>'
    legend = ('<div class="small">的中モデル＝GBM上位3点（<span class="pill pbuy" style="padding:1px 6px">買い</span>'
              '＝自信度上位20%、それ以外は様子見）。高オッズEV＝1-4/1-5・オッズ≥20・EV≥1.2。</div>')
