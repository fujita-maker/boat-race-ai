...
def final_page():
    ...
    css = _DASH_CSS + (
        ...
        # v56: 朝の買い候補リストのタイプ別バッジ(≥40混戦=青 / ≥55=緑 / 安定型=橙)
        ".tg{display:inline-block;font-size:11px;font-weight:700;line-height:1;"
        "padding:4px 7px;border-radius:999px;margin-right:4px;white-space:nowrap;border:1px solid}"
        ".tg40{color:#1f5fa8;background:#eaf2fc;border-color:#bcd6f2}"
        ".tg55{color:#1c7a38;background:#e9f6ee;border-color:#bfe3cc}"
        ".tgst{color:#a35a10;background:#fdf1e3;border-color:#f0d3ae}"
        ".tdtag{text-align:left;white-space:nowrap}")
    head = ('<!doctype html><html lang="ja"><head><meta charset="utf-8">'
            '<meta name="viewport" content="width=device-width,initial-scale=1">'
    ...
    uv_kpi = (_uv_row("≥40（大穴型・過去BT77%）", uv40)
              + _uv_row("≥55（厳選・過去BT97%）", uv55)
              + _uv_row("安定型 キー1着率≥10%（過去BT90%・月別安定）", uv_stable))
    ...
    def _uv_candidates():
        ...
        for jcd in range(1, 25):
            for rno in range(1, 13):
                ...
                # v55: 朝の時点の買い目(暫定)。軸=キー艇は直前でも不変だが、相手P/Q/Rの並びは
                #      展示タイム・進入・STが入ると入れ替わることがある(9/2の144レース実測で約27%)。
                upk = [(p.get("combo"), str(p.get("pos") or ""))
                ...
                cands.append({"closed": closed, "jcd": jcd, "rno": rno,
                              ...
                              "is40": is40, "tiers": tiers, "picks": upk,
        ...
        if n == 0:
            ...
            for c in cands:
                ...
                # v56: タイプを色分けバッジで表示(≥40混戦=青 / ≥55=緑 / 安定型=橙)
                _tgc = {"≥40": "tg40", "≥55": "tg55", "安定型": "tgst"}
                _tgl = {"≥40": "≥40混戦", "≥55": "≥55", "安定型": "安定型"}
                tier_s = "".join('<span class="tg %s">%s</span>' % (_tgc.get(t, "tg40"), _tgl.get(t, t))
                                 for t in c["tiers"])
                h += ('<tr><td>%s</td><td class="ct">%s%sR</td><td class="tdtag">%s</td>'
                ...
                # v55: 買い目(暫定)を1行下に追加(着位置ごとにまとめる)
                if c.get("picks"):
                    grp = {}
                    for cb, pos in c["picks"]:
                        grp.setdefault(pos[:2] or "買い目", []).append(cb)
                    seg = " ／ ".join(
                        "%s %s" % (g, "・".join('<span class="combo">%s</span>' % x for x in v))
                        for g, v in grp.items())
                    h += ('<tr><td colspan="7" class="tsub" '
                          'style="border-top:0;padding-top:0">買い目(暫定)　%s</td></tr>') % seg
            h += '</table>'
        h += ('<div class="tsub" style="margin-top:6px">※「対象」=そのレースが該当する買いタイプ'
              '（<b style="color:#1f5fa8">青=≥40混戦</b>／<b style="color:#1c7a38">緑=≥55</b>'
              '／<b style="color:#a35a10">橙=安定型</b>）。'
             '≥40は混戦(pgap≤0.50)限定・≥55はモーター2連率≥55・安定型はキー1着率≥10%。複数該当ほど強い候補。'
             '最終判定は各レース締切５分前に最新オッズで確定（下の一覧に追加）。pgap0.45超(*)は≥40が直前で見送りに転ぶことあり。'
             '買い目は<b>暫定</b>：軸(キー艇)は直前でも変わらないが、相手2艇の並びは展示タイム・進入が入ると入れ替わることがある'
             '（実測で約4分の1のレース）。オッズは締切5分前に取得するため朝は未表示。'
...
def api_tri_summary():
    ...
    return {"ok": True, "hd": hd, "dates": dates,
            "today": {"hit": agg_hit(today), "uv40": agg_uv(today, "dec40"), "uv55": agg_uv(today, "dec55"),
                      "uv_stable": agg_uv(today, "dec_stable"),
            ...
            "cumulative": {"hit": agg_hit(rows), "uv40": agg_uv(rows, "dec40"), "uv55": agg_uv(rows, "dec55"),
                           "uv_stable": agg_uv(rows, "dec_stable"),
