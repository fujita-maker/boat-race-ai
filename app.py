...
_HO_COLS = ["ts", "date", "jcd", "rno", "closed_at", "combo", "P", "odds",
            "market_p", "EV", "stake", "status", "payout", "ret", "hit"]
    ...
    with _ho_lock:
        ...
        for jcd in range(1, 25):
            for rno in range(1, 13):
                ...
                odds_map = fetch_exacta_odds(jcd, rno, hd)
                if not odds_map:
                    continue          # オッズ未取得の間は保留(次サイクルで再挑戦)
                ...
                if res["decision"] != "買い" or not res.get("picks"):
                    rows.append({**base, "combo": "SKIP", "P": "", "odds": "", "market_p": "",
                                 "EV": "", "stake": 0, "status": "skip", "payout": "", "ret": "", "hit": ""})
                    judged.add(key); made += 1
                    continue
                for p in res["picks"]:
                    rows.append({**base, "combo": p["combo"], "P": p["P"], "odds": p["odds"],
                                 "market_p": p["market_p"], "EV": p["EV"], "stake": 100,
                                 "status": "pending", "payout": "", "ret": "", "hit": ""})
                judged.add(key); made += 1
        # 精算(2連単の結果が出たレース)
        for r in rows:
...
def undervalue_pick(race, odds_map):
    ...
    dec40 = "買い" if (odds_map and gap[X] >= 3 and mot[X] >= 40 and odds_ok and nat_ok) else "見送り"
