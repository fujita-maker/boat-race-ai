...
def final_page():
    ...
    def _uv_candidates():
        ...
        for jcd in range(1, 25):
            for rno in range(1, 13):
                ...
                if not (fnum(k.get("gap")) >= 3 and fnum(k.get("motor2")) >= 40
                        and fnum(k.get("nat2")) >= 5 and pgap <= 0.50):
                    continue
        ...
        h = ('<div class="fcard" style="border-color:#1c7a38">'
             ...
             '&#128994; 本日の ≥４０ 買い候補（混戦 pgap≤0.50・未締切）　%d件</div>' % n)
        if n == 0:
            h += ('<div class="tsub" style="margin-top:6px">今のところ未締切レースに混戦の買い候補はありません'
                  '（本命が抜けていないレースが出れば自動で表示）。</div>')
        else:
            h += '<table style="margin-top:8px"><tr><th>締切</th><th>場R</th><th>キー</th><th>モ/全国2連</th><th>pgap</th><th>状態</th></tr>'
            for c in cands:
                bd = (c["pgap"] > 0.45)
                ...
                h += ('<tr><td>%s</td><td class="ct">%s%sR</td>'
                    ...
                    c["closed"].strftime("%H:%M"), ven(c["jcd"]), c["rno"],
                    c["frame"], round(fnum(c["keyp"]) * 100), round(fnum(c["motor2"])),
                    round(fnum(c["nat2"])), c["pgap"], ("*" if bd else ""), st)
            h += '</table>'
        h += ('<div class="tsub" style="margin-top:6px">※pgap≤0.50＝本命が抜けていない混戦だけ≥４０を買う方式。'
             '最終判定は各レース締切５分前に最新オッズで確定（下の一覧に追加）。pgap0.45超(*)は直前で見送りに転ぶことあり。'
             '過去半年BT 91.9%＝依然100%未満の検証ツール。</div></div>')
        return h
