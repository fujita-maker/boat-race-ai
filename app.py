...
_MIDEV_DISPLAY = "midev_sel"      # v72: v71の "midev_ev1ev2" から絞り込み版に変更
...
def final_page():
    ...
    uv_live = ('<div class="fcard" style="border-color:#1c7a38;background:#f4fbf6">'
               ...
               '&#9989; 実際に買うのはこの3つだけ（下の各レースのカードで「🟢 買い」になるのもこの3つだけ）</div>'
    ...
    def card(r):
        ...
        _planB = (uv.get("dec40") == "買い")
        _planA = (uv.get("dec_stable") == "買い" and _rk not in _A_STOPPED)
        ...
        _plans = [lab for ok, lab in ((_planA, "A案"), (_planB, "B案"), (bool(_cpk), "C案")) if ok]
        isbuy = bool(_plans)
        ...
        if isbuy:
            ...
            h += '<div class="tsub" style="margin:4px 0">A案・B案・C案とも条件に当てはまらないため買いません。%s</div>' % (
                '（安定型は該当したが、当日5連敗ストップ中のため買わない）' if _planA_stopped else '')
        ...
        if uv:
            ...
            badge = ('過小評価キー　' + _bdg("B案(≥40):", d40) + ' ／ ' + _bdg("A案(安定型):", dst)
                     + ' ／ ' + _bdg("≥55(記録のみ):", d55))
            inner = ""
            k = uv.get("key")
            if k:
    ...
    def _cum_block():
        ...
        specs += [(("C案 買い(表示中) " if nm == _MIDEV_DISPLAY else "C案 比較 ") + lab, (lambda nm=nm: _midev_pairs_all(nm)))
                  for nm, lab in _MIDEV_SHADOWS if (not _MIDEV_DISPLAY or nm in (_MIDEV_DISPLAY, "midev_ev1ev2"))]
    ...
    def _rare_block(field="rare", thr=10.0):
        ...
        if field == "rare":
            h += ('<div class="tsub" style="margin-top:6px"><b>見方：</b>朝から該当レースと買い目を出し、'
                  ...
                  '3位以下なら「🟢買い候補」に切り替わる。過去258日の該当211Rでは、1号艇の1着率が'
                  '展示1〜2位 57%（普通の1号艇並み）／3〜4位 42%／5〜6位 37%。'
                  'ただし6点買いの的中は211Rで6本しかなく、展示で分けた回収率はまだ判断できない。</div>')
            ...
            h += ('<div class="tsub" style="margin-top:6px"><b>見方：</b>≤10%版と同じく、'
                  '<b>展示タイムで1号艇が2位以内なら「🔴見送り」、3位以下なら「🟢買い候補」</b>（公開前は「展示待ち」）。'
                  '過去258日の該当レースでは、1号艇の1着率が展示1〜2位 57%（915R）／3〜4位 46%（665R）／5〜6位 42%（468R）。'
                  'ただしこの拡張版は展示で絞っても回収率は100%に届かない（展示3位以下だけ買っても約40%）＝記録専用。</div>')
    ...
    return HTMLResponse(head + bar + uv_kpi + cum_block + midev_block + midev_today + rare10_block + cand_block + rare_block + body + kpi_ref + note + "</body></html>")
