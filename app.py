...
def _midev_line(uv):
    ...
    if _MIDEV_DISPLAY:
        return ('<div class="tsub" style="margin-top:2px">C案 厳選<span style="color:#8a97a5">（EV1位＋EV≥%s の1点×%s円）</span>'
                '　%s</div>') % (_MIDEV_SH_EV, _MIDEV_UNIT, body)
...
_MIDEV_N = 5                        # 最大点数
_MIDEV_UNIT = 200                   # 1点あたりの実際の賭け金(少額・検証中のため)
...
def final_page():
    ...
    uv_live = ('<div class="fcard" style="border-color:#1c7a38;background:#f4fbf6">'
               ...
               '&#9989; 実際に買うのはこの2つだけ（1レース4点×1,000円＝4,000円）</div>'
               + _uv_row("B案：≥40混戦（過小評価キー）", uv40, live=True)
               + _uv_row("A案：安定型＋5連敗ストップ", uv_stable_stop, live=True)
               + (_uv_row("C案：中オッズEV 厳選（EV1位＋EV≥%s の1点×%d円・検証中）"
                          % (_MIDEV_SH_EV, _MIDEV_UNIT), uv_midev_sh[_MIDEV_DISPLAY], live=True)
               ...
               + '<div class="tsub" style="margin-top:8px">'
                 'この2案を並行してライブ検証中（実運用ルールv54）。'
                 '両方に「買い」が出たレースは、より強い候補という意味で同じ買い目を1回だけ買う。'
                 '下の「記録のみ」は<b>買いません</b>。</div></div>')
    uv_shadow = ('<div class="fcard" style="border-color:#dbe2ea">'
                 '<div style="font-weight:700;color:#5c6b7a;font-size:15px">'
                 '&#128203; 記録のみのバケット（買わない・検証用）　%d件</div>' % (8 if _MIDEV_DISPLAY else 8 + len(_MIDEV_SHADOWS))
                 ...
                 + _uv_row("≥55（厳選）", uv55)
                 + _uv_row("安定型フラット（ストップ無し・A案の素の数字）", uv_stable)
    ...
    def _uv_candidates():
        ...
        if n == 0:
            ...
            h += ('<table style="margin-top:8px"><tr><th>締切</th><th>場R</th><th>対象</th>'
                  ...
                  '<th style="text-align:left">買い目（朝の時点・4点×1,000円=4,000円）</th><th>状態</th></tr>')
            for c in cands:
    ...
    def _uv_pairs_all(field):
        ...
        for r in rows:
            ...
            if not u:
                continue
            if u.get(field, u.get("decision")) != "買い":
                continue
    ...
    def _midev_today_block_disp():
        """v71: 本日のC案 厳選(EV1位＋EV≥2.0の1点)だけの一覧。上限なし・締切順。"""
        items = []
        for r in today:
            try:
                u = json.loads(r.get("uv_json") or "{}")
        ...
        h = ('<div class="fcard" style="border-color:#1c7a38">'
             '<div class="tsub" style="font-weight:700;color:#1c7a38">'
             '&#128994; 本日の C案 厳選買い目（EV1位＋EV≥%s の1点×%d円）　%d レース'
             '<span style="font-weight:400">　精算済 %d／的中 %d／投資 %s円・払戻 %s円</span></div>') % (
            _MIDEV_SH_EV, _MIDEV_UNIT, len(items), sum(1 for x in items if x[5]), hits,
            f"{int(stake):,}", f"{int(ret):,}")
        if not items:
            h += '<div class="tsub" style="margin-top:6px">まだ該当レースがありません。</div></div>'
            return h
        h += ('<table style="margin-top:8px"><tr><th>締切</th><th>場R</th>'
              '<th style="text-align:left">買い目（組番・オッズ・EV）</th><th>金額</th><th>結果</th></tr>')
        for r, mv, sub, s_, r_, st_ in items:
            ...
            h += ('<tr><td>%s</td><td class="ct">%s%sR</td>'
                  ...
                  '<span class="tsub">(%s倍・EV%s)</span></td><td>%s円</td><td>%s</td></tr>') % (
                r.get("closed_at", ""), ven(int(fnum(r.get("jcd")))), r.get("rno"),
                p.get("combo"), p.get("odds"), p.get("ev"), f"{int(s_):,}", res)
        h += '</table>'
        sc = uv_midev_disp_cum or {"bset": 0, "hits": 0, "roi": 0}
        h += ('<div class="tsub" style="margin-top:6px">'
              '市場オッズ%d〜%d倍・較正後EV≥%sの候補のうち、<b>EVが一番高い1点が EV≥%s の時だけ</b>1点%d円。'
              '1日の上限は設けていません（ライブ実績は1日15〜40レース前後＝最大8,000円程度）。<br>'
              '通算（9/08〜・精算済 %d レース）：的中 %d 本・回収率 %.1f%%。<br>'
              '<b style="color:#b23b3b">まだ勝ちは証明されていません。</b>'
              'この条件はライブ8日の後付け分析で見つけたもので、8月の別の5日間(本番ロジック再現)では回収率81.8%%に崩れました。'
              'ポセイドンの過去オッズ(8/21〜)で別期間の検証を増やしている最中です。'
              '元の条件（全点・早い者順10レース）の記録は裏で続けています。</div></div>') % (
            int(_MIDEV_LO), int(_MIDEV_HI), _MIDEV_EV, _MIDEV_SH_EV, _MIDEV_UNIT,
    ...
    def _cum_block():
        ...
        specs += [(("C案 厳選 " if nm == _MIDEV_DISPLAY else "影 C案 ") + lab, (lambda nm=nm: _midev_pairs_all(nm)))
                  for nm, lab in _MIDEV_SHADOWS if (not _MIDEV_DISPLAY or nm == _MIDEV_DISPLAY)]
    ...
    def _rare_block(field="rare", thr=10.0):
        ...
        data = _rare_data
        title = ("&#127775; 希少・波乱の目（1号艇モーター激弱 ≤10%＝約1/180の希少）" if field == "rare" else
                 "&#127775; 波乱の目・拡張版（1号艇モーター ≤20%＝1日約8件・記録のみ）")
        ...
        if n == 0:
            ...
            h += ('<table style="margin-top:8px"><tr><th>締切</th><th>場R</th><th>1号艇モ</th>'
                  '<th>買い目(最強機力頭6点=600円・%s)</th><th>結果</th></tr>'
                  % ("記録のみ" if field != "rare" else "最小額"))
            for c in cands:
                rr = c["rr"]; combos = " / ".join(p["combo"] for p in rr["picks"])
                res = "—"
                if c["led"]:
                    lr, lrr = c["led"]
                ...
                h += ('<tr><td>%s</td><td class="ct">%s%sR</td><td>%.1f%%%s</td>'
                      '<td class="combo" style="font-size:12px">%s</td><td>%s</td></tr>') % (
                    (c["closed"].strftime("%H:%M") if c["closed"] else "-"), ven(c["jcd"]), c["rno"],
                    rr["mot1"], both, combos, res)
            h += '</table>'
        if field == "rare":
            h += ('<div class="tsub" style="margin-top:6px">※あなたの着眼（人気の1号艇がモーター激弱→波乱）を前向き記録する検証枠。'
                  '<b>過去258日(2026-01-01〜09-15)は該当213レース・的中6本(2.8%)・回収率142.6%だが、'
                  '14万4,070円の1本を含む大穴3本を抜くと10.4%＝1発次第の完全ノイズ＝買い非推奨</b>'
                  '（激弱でも1号艇は47%勝つ）。通算は /api/tri/summary の rare_upset で自動集計。</div></div>')
        else:
            h += ('<div class="tsub" style="margin-top:6px">※v70(9/16)追加の拡張版。<b>お金は入れない記録のみ</b>。'
                  '<b>過去258日の検証: 該当2,070レース(1日約8件)・的中54本(2.6%)・回収率52.6%・大穴3本抜き32.2%・'
                  '9か月すべて100%割れ</b>。10〜20%帯だけだと1,857レース・42.3%。'
                  '1号艇の1着率は50.0%（全レース平均54.9%より約5pt低いだけ）で、「1が飛ぶ」ほどには弱くならず、'
                  '市場もその程度は織り込み済み＝≤10%より明確に悪い。前向きに同じ結果になるかを記録する。'
                  '通算は /api/tri/summary の rare_upset20 で自動集計。</div></div>')
        return h
    rare_block = _rare_block("rare", 10.0) + _rare_block("rare20", 20.0)
    ...
    return HTMLResponse(head + bar + kpi + uv_kpi + cum_block + midev_block + midev_today + cand_block + rare_block + body + note + "</body></html>")
...
def _rare_upset_pick(rc, thr=10.0):
    # 🌟希少・波乱の目: 1号艇の全国モーター2連率が極端に低い(≤thr%=全体の約0.7%=希少)時に、
    # 最強機力(2-6号艇)頭で上位3機力を2-3着に流す6点。過去BT 67〜80%(=買い非推奨)だが、
    # ユーザー希望で希少ケースだけ前向き記録する検証枠。実際の買い(dec40等)には影響しない。
    try:
        mm = _uv_extract(rc)
    except Exception:
        return {"flag": False}
    ...
    head = others[0]; legs = others[1:4]   # headを除く2〜4番目の機力を2-3着に流す=6点
    combos = []; seen = set()
    for a in legs:
        for b in legs:
            if a != b:
                cs = f"{head}-{a}-{b}"
                if cs not in seen:
                    seen.add(cs); combos.append({"combo": cs, "stake": 100})
    return {"flag": True, "mot1": round(mot1, 1), "head": head,
            "motors": {f: round(m(f), 1) for f in range(1, 7)}, "picks": combos, "total": len(combos) * 100}
...
def undervalue_pick(race, odds_map):
    ...
    for (ijk, stake, pos) in combos:
        ...
        picks_all.append(dict(rec, bet=(pos not in _UV_DROP)))
        if pos in _UV_DROP:
