    ...
    with _ho_lock:
        ...
        for jcd in range(1, 25):
            for rno in range(1, 13):
                ...
                if conf < 0.12 or gap < 0.30:
                    rows.append({**base, "combo": "SKIP", "P": "", "odds": "", "market_p": "",
                                 "EV": "", "stake": 0, "status": "skip", "payout": "", "ret": "", "hit": "", "model_buy": 0})
                    judged.add(key); made += 1
                    continue
                # ゲート通過: 1-4/1-5 を(オッズ/EV問わず)両方記録し、しきい値検証をライブで可能にする。
                # model_buy=1 = 現行モデルが実際に買う条件(odds>=20 & EV>=1.2 & P>=0.05)。0=影(記録のみ)。
                ...
                for (i, j) in ((1, 4), (1, 5)):
                    ...
                    rows.append({**base, "combo": cs, "P": round(p, 4), "odds": o,
