...
st.set_page_config(page_title="WDJ Boat Race AI v8", layout="wide")
...
# ---------- UI ----------
st.title("🚤 WDJ ボートレースAI Web版 v8")
st.caption(
    "公式・梅吉AI・ポセイドンを統合。取得失敗でも参考予想は必ず表示し、"
    "WDJ115-v2条件に適合したレースだけ5,000円で仮想購入します。"
)
st.info("V8安定版：外部サイトが重い・停止・HTML変更の場合でも、アプリを落とさず参考買い目を表示します。")
    ...
    if st.button("3サイト取得 → 予想固定", type="primary", use_container_width=True):
        ...
        with st.spinner("公式・梅吉AI・ポセイドンを取得し、予想中…"):
            ...
            try:
                ...
                # どのサイトが想定外でも画面を落とさず、枠順ベースの参考予想を返す
                safe_official = {
        ...
        if calc["action"] == "買い":
            ...
            st.warning("購入は見送りですが、上の参考買い目は予想結果として固定しています。")
