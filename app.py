...
def final_page():
    ...
    def _uv_candidates():
        ...
        if n == 0:
            ...
            h += ('<table style="margin-top:8px"><tr><th>締切</th><th>場R</th><th>対象</th>'
                  ...
                  '<th style="text-align:left">買い目（朝の時点・6点=1,000円）</th><th>状態</th></tr>')
            for c in cands:
        ...
        h += ('<div class="tsub" style="margin-top:6px">※「対象」=そのレースが該当する買いタイプ。'
             ...
             '<b>「買い目」は朝の時点の予定（v59の6点＝1,000円／キーを1着に置く2点は除外済み）。</b>'
    ...
    return HTMLResponse(head + bar + kpi + uv_kpi + cand_block + rare_block + body + note + "</body></html>")
...
def undervalue_pick(race, odds_map):
    ...
    _UV_DROP = ("1着(上振れ)", "1着(裏)")
    tri = trifecta_probs_pl(power)
    picks = []
    exp_ret = 0.0; total = 0            # 実際に賭ける6点
