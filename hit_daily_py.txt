...
def topn_conf(vk, topn):
    """1レースの強さv(6,) → 上位topn点[(idx,prob)...] と 自信度(上位topn確率和)。"""
    pr = []
    for i in range(6):
        v2 = vk.copy(); v2[i] = -1e9; p2 = smax(v2[None])[0]
        for j in range(6):
            if i != j:
                pr.append(((i, j), float(smax(vk[None])[0][i] * p2[j])))
    pr.sort(key=lambda t: t[1], reverse=True)
    picks = pr[:topn]
    return picks, sum(p for _, p in picks)
...
def main():
    ...
    ap.add_argument("--stake", type=float, default=1000.0)   # 1点1000円(3点なら3000円)
    ap.add_argument("--ledger", default="data/hit_ledger.csv")
    ap.add_argument("--conf-quantile", type=float, default=0.80,
                    help="この分位以上の自信度レースだけ買う(0.80=自信度上位20%)。0で見送り無し")
    a = ap.parse_args()
    ...
    # --- 見送りゲート: 学習データの自信度分布から しきい値(上位20%)を自動算出 ---
    conf_thr = 0.0
    if a.conf_quantile and a.conf_quantile > 0:
        pw_tr = gb.predict_proba(Xa_tr.reshape(-1, Fa))[:, 1].reshape(len(tr), 6)
        v_tr = np.log(np.clip(pw_tr, 1e-6, 1))
        confs_tr = np.array([topn_conf(v_tr[k], a.topn)[1] for k in range(len(tr))])
        conf_thr = float(np.quantile(confs_tr, a.conf_quantile))
        print(f"[hit] 見送りゲート: 自信度>={conf_thr:.3f} のレースだけ買う"
              f"(学習分布の上位{int((1-a.conf_quantile)*100)}%)", flush=True)
    ...
    logged = 0; bought = 0; skipped = 0
    for k, r in enumerate(today):
        vk = v[k]
        picks, conf = topn_conf(vk, a.topn)
        if conf < conf_thr:          # 見送りゲート: 自信度不足→買わない
            skipped += 1
            continue
        bought += 1
        b = r["boats"]
    ...
    # 今日ぶんだけの集計
    td = [r for r in s if str(r["date"]) == str(hd)]
    tstk = sum(float(r["stake"]) for r in td); tret = sum(float(r["ret"] or 0) for r in td)
    tbyrace = {}
    for r in td:
        kk = (r["jcd"], r["rno"]); tbyrace[kk] = tbyrace.get(kk, 0) or int(float(r["hit"] or 0))
    troi = (tret / tstk * 100) if tstk else 0
    print(f"[hit] hd={hd} 当日{len(today)}確定 / 買い{bought}R 見送り{skipped}R / 記録{logged}点(上位{a.topn}) @1点{a.stake:,.0f}円", flush=True)
    print(f"[今日] レース{len(tbyrace)} 的中{sum(tbyrace.values())} "
          f"投資{tstk:,.0f}円 払戻{tret:,.0f}円 純益{tret-tstk:,.0f}円 回収率{troi:.1f}%", flush=True)
