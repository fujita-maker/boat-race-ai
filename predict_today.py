...
def topn_conf(vk, topn):
    """強さv(6,) → 上位topn点[(idx,prob)...] と 自信度(上位topn確率和)。hit_dailyの版に依存しない自前実装。"""
    pr = []
    for i in range(6):
...
def fetch_exacta_odds(jcd, rno, hd, retries=2, timeout=10):
...
def main():
    ...
    if len(tr) >= 500:
        ...
        confs = np.array([topn_conf(v_tr[k], a.topn)[1] for k in range(len(tr))])
    ...
    for jcd in H.TARGET_VENUES:
        for rno in range(1, 13):
            ...
            if gb is not None:
                ...
                picks, conf = topn_conf(v, a.topn)
                combos = [f"{boats_n[i]}-{boats_n[j]}" for (i, j), _ in picks]
                gate = conf >= conf_thr
                rec["hit"] = {"combos": combos, "conf": round(float(conf), 3), "buy": bool(gate)}
                if gate:
                    hit_go += 1
            # 高オッズモデル: 先にゲート判定(オッズ不要)。通過レースだけオッズ取得(高速化)。
            boats = extract_boats(rc)
            power = predict_power(boats)
            hconf, hgap = _conf_gap(power)
            if hconf < 0.12 or hgap < 0.30:
                res = {"decision": "見送り", "picks": []}
            else:
